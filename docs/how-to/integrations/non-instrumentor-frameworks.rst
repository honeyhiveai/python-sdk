...
without relying on auto-instrumentation libraries.
...
.. code-block:: python
   ...
   # Then initialize your framework
...
.. code-block:: python
   ...
   tracer = HoneyHiveTracer.init(api_key="key", project="proj")
   framework = YourFramework()
...
.. code-block:: bash
   ...
   export HH_PROJECT="my-project"  # Required for OTLP tracing
...
HoneyHive adds minimal overhead to span processing:

- Target: <1ms per span
- Actual: Varies based on configuration
- Use performance benchmarks to measure impact

Memory Usage
~~~~~~~~~~~~

Monitor memory usage with multiple frameworks:

- Expected overhead: <5% for single framework
- Multiple frameworks: <15% overhead
- Use memory profiling tools for analysis

Concurrent Performance
~~~~~~~~~~~~~~~~~~~~~~

HoneyHive is designed for concurrent operations:

- Thread-safe span processing
- Minimal contention between frameworks
...
- ``strands_integration.py`` - AWS Strands integration
...
- :doc:`../../development/testing/real-api-testing` - Testing with real APIs
