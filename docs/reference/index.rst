...
- **Framework Examples**: Integration examples for OpenAI Agents (SwarmAgent), DSPy (signatures and optimization), AWS Bedrock (Nova/Titan/Claude models), AWS Strands (TracerProvider pattern), Google ADK (async support), LangGraph (state workflows), Pydantic AI (type-safe agents), and more
