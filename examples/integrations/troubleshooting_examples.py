...
def generate_troubleshooting_report():
    ...
    try:
        ...
        if hasattr(current_provider, '_span_processors'):
            ...
            if processor_count == 0:
                report["recommendations"].append("No span processors found - spans may not be exported")
    ...
    if not report["environment"]["HH_PROJECT"]:
        report["recommendations"].append("Set HH_PROJECT environment variable (required for OTLP)")
    
    if report["environment"]["HH_OTLP_ENABLED"] == "false":
        report["recommendations"].append("OTLP is disabled - spans will not be exported to HoneyHive")
