    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "user",
                    "content": "Explain OpenLLMetry in one sentence."
                }
            ],
            max_tokens=100,
            temperature=0.7
...
def multi_deployment_azure_workflow(prompts: List[str]) -> Dict[str, Any]:
    ...
    deployments = [
        ...
        os.getenv("AZURE_OPENAI_GPT4_TURBO_DEPLOYMENT", "gpt-4-turbo")  # Optional GPT-4 Turbo
    ]
    
    # Filter out None/empty deployments
    available_deployments = [d for d in deployments if d and d != "gpt-4" and d != "gpt-4-turbo"]
