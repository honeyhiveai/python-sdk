...
class CustomAIFramework:
    ...
    def process_text(self, text: str, operation_type: str = "analysis") -> Dict[str, Any]:
    ...
    def batch_process(self, texts: List[str], batch_size: int = 5) -> List[Dict[str, Any]]:
        ...
        with self.tracer.start_as_current_span(f"{self.name}.batch_process") as span:
            ...
            span.set_attribute("batch.count", (len(texts) + batch_size - 1) // batch_size)
            ...
            for i in range(0, len(texts), batch_size):
                ...
                with self.tracer.start_as_current_span(f"batch_{batch_num}") as batch_span:
                    batch_span.set_attribute("batch.number", batch_num)
                    batch_span.set_attribute("batch.items", len(batch))
                    
                    batch_results = []
                    for j, text in enumerate(batch):
                        with self.tracer.start_as_current_span(f"item_{j+1}") as item_span:
...
def demonstrate_honeyhive_first_integration():
    ...
    # Single operation
    result1 = framework.process_text("This is a good example of integration!", "sentiment")
    print(f"   Single operation result: {result1['sentiment']} ({result1['confidence']})")
...
def demonstrate_framework_first_integration():
    ...
    print(f"   ✅ HoneyHive integrated with existing provider (Session: {tracer.session_id})")
...
def demonstrate_multi_framework_integration():
    ...
    with otel_tracer.start_as_current_span("multi-framework-workflow") as workflow_span:
        ...
        with otel_tracer.start_as_current_span("framework-a-processing") as fa_span:
            ...
            result_a = framework_a.process_text("Initial text for processing", "initial")
    ...
    print(f"   Workflow completed: {result_a['sentiment']} → {result_b['sentiment']} → {result_c['sentiment']}")
...
def demonstrate_concurrent_operations():
    ...
    print(f"   Average per operation: {(end_time - start_time) / len(all_results):.3f}s")
