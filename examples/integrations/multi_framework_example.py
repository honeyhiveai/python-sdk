...
class WorkflowOrchestrator:
    ...
    def execute_sequential_workflow(self, input_data: str) -> Dict[str, Any]:
        ...
        with self.tracer.start_as_current_span("sequential-workflow") as workflow_span:
            ...
            if "framework_a" in self.frameworks:
                with self.tracer.start_as_current_span("step-1-framework-a") as step_span:
                    ...
                    current_data = f"processed_by_a_{result_a.get('operation', 'unknown')}"
                    step_span.set_attribute("step.result", "success")
            
            # Step 2: Framework B - Secondary processing
            if "framework_b" in self.frameworks:
                with self.tracer.start_as_current_span("step-2-framework-b") as step_span:
            ...
            if "framework_c" in self.frameworks:
                with self.tracer.start_as_current_span("step-3-framework-c") as step_span:
    ...
    def execute_parallel_workflow(self, input_data: List[str]) -> Dict[str, Any]:
        ...
        with self.tracer.start_as_current_span("parallel-workflow") as workflow_span:
            ...
            def process_with_framework(framework_name: str, framework: Any, data: str) -> Dict[str, Any]:
                """Process data with a specific framework."""
                with self.tracer.start_as_current_span(f"parallel-{framework_name}") as span:
                    ...
                    span.set_attribute("parallel.result", "success" if "error" not in result else "error")
                    return {
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=len(self.frameworks)) as executor:
                futures = []
                
                for i, (framework_name, framework) in enumerate(self.frameworks.items()):
                    data_item = input_data[i % len(input_data)]  # Cycle through input data
                    future = executor.submit(process_with_framework, framework_name, framework, data_item)
            ...
            workflow_span.set_attribute("workflow.parallel_results", len(parallel_results))
            workflow_span.set_attribute("workflow.status", "completed")
            
            self.results.append(final_result)
            return final_result
    
    def execute_hybrid_workflow(self, input_data: str, parallel_data: List[str]) -> Dict[str, Any]:
        ...
        with self.tracer.start_as_current_span("hybrid-workflow") as workflow_span:
            ...
            with self.tracer.start_as_current_span("phase-3-integration") as phase3_span:
                integration_data = {
                    "sequential_output": sequential_result["final_data"],
                    "parallel_outputs": [r["result"] for r in parallel_result["parallel_results"]],
                ...
                if self.frameworks:
                    ...
                    if hasattr(framework, 'execute_operation'):
                        ...
                        integration_result = {"status": "integrated", "method": "manual"}
...
def demonstrate_basic_multi_framework():
    ...
    for name, framework in frameworks.items():
        ...
        if hasattr(framework, 'execute_operation'):
            result = framework.execute_operation(f"multi_test_{name}", framework_type=name)
        elif hasattr(framework, 'process_data'):
            result = framework.process_data(f"multi_data_{name}", "multi_test")
        elif hasattr(framework, 'analyze_content'):
            result = framework.analyze_content(f"multi content {name}", "multi_analysis")
...
def demonstrate_workflow_orchestration():
    ...
    print(f"   ✅ Sequential workflow completed: {sequential_result['steps_completed']} steps")
    ...
    print(f"   ✅ Parallel workflow completed: {parallel_result['frameworks_used']} frameworks")
...
def demonstrate_context_propagation():
    ...
    with otel_tracer.start_as_current_span("user-request") as user_span:
        ...
        with otel_tracer.start_as_current_span("response-generation") as response_span:
            ...
            response_span.set_attribute("response.result", result_c.get("status", "unknown"))
...
def demonstrate_performance_monitoring():
    ...
    for framework_name, framework in frameworks.items():
        ...
        for i in range(operations_per_framework):
            ...
            if hasattr(framework, 'execute_operation'):
                result = framework.execute_operation(f"perf_op_{i}", framework=framework_name)
            elif hasattr(framework, 'process_data'):
        ...
        print(f"   {framework_name}: {operations_per_framework} ops in {framework_duration:.3f}s "
