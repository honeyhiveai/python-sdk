...
try:
    ...
    print("⚠️  Google AI SDK not available. Install with: pip install google-generativeai")
    print("   This example will show the integration pattern but won't make actual API calls")
...
def demonstrate_basic_integration(tracer: HoneyHiveTracer):
    ...
    with tracer.start_span("google_ai_generation") as span:
        try:
            ...
            span.set_attribute("honeyhive.input_tokens", len("Hello! How are you today?"))
...
def demonstrate_vanilla_tracer_integration(tracer: HoneyHiveTracer):
    ...
    with tracer.start_span("custom_google_ai_workflow") as span:
        ...
        try:
            # Make API call
            response = model.generate_content("Write a short Python function to calculate fibonacci numbers")
...
def demonstrate_advanced_span_management(tracer: HoneyHiveTracer):
    ...
    with tracer.start_span("advanced_google_ai_workflow") as parent_span:
        ...
        try:
            ...
            with tracer.start_span("step_1_generation", parent_id=parent_span.get_span_context().span_id) as step1_span:
                step1_span.set_attribute("honeyhive.step", "initial_generation")
                response1 = model.generate_content("Explain quantum computing in simple terms")
                step1_span.set_attribute("honeyhive.response_length", len(response1.text))
                print(f"✓ Step 1 completed: {response1.text[:80]}...")
            
            # Step 2: Refine the content
            with tracer.start_span("step_2_refinement", parent_id=parent_span.get_span_context().span_id) as step2_span:
                ...
                step2_span.set_attribute("honeyhive.response_length", len(response2.text))
                print(f"✓ Step 2 completed: {response2.text[:80]}...")
            
            # Step 3: Create summary
            with tracer.start_span("step_3_summary", parent_id=parent_span.get_span_context().span_id) as step3_span:
...
def demonstrate_chat_conversation(tracer: HoneyHiveTracer):
    ...
    with tracer.start_span("chat_conversation") as chat_span:
        ...
        try:
            ...
            # First message
            with tracer.start_span("chat_message_1", parent_id=chat_span.get_span_context().span_id) as msg1_span:
                ...
                msg1_span.set_attribute("honeyhive.response_length", len(response1.text))
                print(f"✓ Message 1: {response1.text[:80]}...")
            
            # Second message
            with tracer.start_span("chat_message_2", parent_id=chat_span.get_span_context().span_id) as msg2_span:
                ...
                msg2_span.set_attribute("honeyhive.response_length", len(response2.text))
...
def demonstrate_session_enrichment(tracer: HoneyHiveTracer):
    ...
    try:
        ...
        enrichment_data = {
            ...
            "feedback": {
                "rating": 5,
                "comment": "Excellent integration example"
            },
            "metrics": {
                "total_api_calls": 5,
