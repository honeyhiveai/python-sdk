...
try:
    ...
    print("⚠️  OpenInference not available. Install with: pip install openinference-instrumentation-openai")
    print("   This example will show the integration pattern but won't run the actual instrumentation")
    ...
    try:
        ...
        if tracer.session_id:
            event_id = tracer.create_event(
                ...
                inputs={"messages": [{"role": "user", "content": "Hello! How are you today?"}]},
                outputs={"response": response.choices[0].message.content, "usage": response.usage.model_dump()},
            ...
            if OPENAI_AVAILABLE:
                ...
                if tracer.session_id:
                    event_id = tracer.create_event(
                        ...
                        outputs={"response": response.choices[0].message.content, "usage": response.usage.model_dump()},
                        metadata={"model": "gpt-3.5-turbo", "operation": "quantum_explanation"}
                    )
                    print(f"✓ HoneyHive event created: {event_id}")
            else:
        ...
        with tracer.start_span("complex_ai_workflow") as parent_span:
            ...
            with tracer.start_span("step_1_generate") as step1_span:
                ...
                if OPENAI_AVAILABLE:
                    ...
                    response1 = client.chat.completions.create(
                        model="gpt-3.5-turbo",
                        messages=[{"role": "user", "content": "What are the main benefits of AI?"}],
                        max_tokens=80
                    )
                    
                    step1_span.set_attribute("openai.model", "gpt-3.5-turbo")
                    if response1.usage:
                        step1_span.set_attribute("openai.tokens_used", response1.usage.total_tokens)
                    step1_span.set_attribute("step.result", "success")
                    
                    print(f"✓ Step 1 completed: {response1.choices[0].message.content[:50]}...")
                    ...
                    if tracer.session_id:
                        event_id = tracer.create_event(
                            ...
                            outputs={"response": response1.choices[0].message.content, "usage": response1.usage.model_dump()},
            ...
            with tracer.start_span("step_2_refine") as step2_span:
                ...
                if OPENAI_AVAILABLE:
                    ...
                    if response2.usage:
                        step2_span.set_attribute("openai.tokens_used", response2.usage.total_tokens)
                    step2_span.set_attribute("step.result", "success")
                    
                    print(f"✓ Step 2 completed: {response2.choices[0].message.content[:50]}...")
                    ...
                    if tracer.session_id:
                        event_id = tracer.create_event(
                            ...
                            outputs={"response": response2.choices[0].message.content, "usage": response2.usage.model_dump()},
            ...
            if tracer.session_id:
                event_id = tracer.create_event(
                    ...
                    metadata={"workflow_status": "completed", "workflow_type": "multi_step_ai_processing"}
                )
