...
def measure_bulk_sdk_operations(num_requests: int = 50, spans_per_request: int = 10, work_per_span_ms: float = 20) -> Dict[str, Any]:
    ...
    return {
        ...
        "std_dev_ms": statistics.stdev(request_measurements) if len(request_measurements) > 1 else 0,
        "coefficient_of_variation": (
            statistics.stdev(request_measurements) / statistics.mean(request_measurements) * 100
            if statistics.mean(request_measurements) > 0 else 0
        ),
...
def measure_detailed_sdk_operations(iterations: int = 1000, work_per_iteration_ms: float = 1.0) -> Dict[str, Any]:
    ...
    flush_times = []
    for _ in range(min(100, iterations // 10)):  # Reasonable number of flush measurements
        start_time = time.perf_counter()
    ...
    try:
        ...
        if test_type == "bulk":
            ...
            def calc_stats(values: List[float]) -> Dict[str, float]:
                if not values:
                    return {"mean": 0, "median": 0, "std_dev": 0, "min": 0, "max": 0, "cv": 0}
                ...
                return {
                    ...
                    "min": min(values),
                    "max": max(values),
                    "cv": (statistics.stdev(values) / mean_val * 100) if mean_val > 0 else 0,
                }
            ...
            results = {
                ...
                "overhead_analysis": {
                    ...
                    "overhead_vs_work_percentage": (
                        statistics.mean(sdk_measurements["total_overhead"]) / statistics.mean(sdk_measurements["work_times"]) * 100
                        if statistics.mean(sdk_measurements["work_times"]) > 0 else 0
                    ),
                    "coefficient_of_variation": calc_stats(sdk_measurements["total_overhead"])["cv"],
                },
