...
class TestComplexModelScenarios:
    ...
    def test_create_event_request_with_nested_config(self):
        ...
        try:
            ...
            assert event_request.inputs["metadata"]["task_type"] == "information_extraction"
            ...
            assert serialized["inputs"]["metadata"]["task_type"] == "information_extraction"
    ...
    def test_create_datapoint_request_with_nested_history(self):
        ...
        try:
            ...
            assert datapoint_request.ground_truth["evaluation_criteria"]["relevance"] == 0.95
            assert datapoint_request.inputs["user_preferences"]["location"] == "San Francisco"
            ...
            assert serialized["ground_truth"]["evaluation_criteria"]["relevance"] == 0.95
    ...
    def test_post_configuration_request_with_nested_parameters(self):
        ...
        try:
            from honeyhive.models.generated import (
                Parameters, Parameters2, CallType, FunctionCallParams, EnvEnum, SelectedFunction
            )
            ...
            assert config_request.parameters.hyperparameters["advanced"]["beam_search"]["enabled"] is True
            assert len(config_request.parameters.selectedFunctions) == 2
            assert config_request.parameters.selectedFunctions[0].name == "extract_entities"
            ...
            assert serialized["parameters"]["hyperparameters"]["advanced"]["beam_search"]["enabled"] is True
            assert serialized["parameters"]["selectedFunctions"][0]["name"] == "extract_entities"
    ...
    def test_create_run_request_with_complex_evaluation_config(self):
        ...
        try:
            ...
            run_request = CreateRunRequest(
                ...
                event_ids=[UUIDType("event-1"), UUIDType("event-2"), UUIDType("event-3")],
            ...
            assert run_request.configuration["evaluation_settings"]["parallel_processing"] is True
            assert "json" in run_request.configuration["output_format"]["export_formats"]
            assert run_request.metadata["quality_metrics"]["inter_evaluator_reliability"] == 0.89
            
            # Test serialization
            serialized = run_request.model_dump(exclude_none=True)
            assert serialized["configuration"]["metrics"]["accuracy"]["threshold"] == 0.8
            assert serialized["configuration"]["evaluation_settings"]["parallel_processing"] is True
    ...
    def test_update_tool_request_with_complex_changes(self):
        ...
        try:
            ...
            # Verify complex nested structures
            assert update_request.parameters["new_capabilities"]["sentiment_analysis"] is True
            assert update_request.parameters["new_capabilities"]["entity_recognition"] is True
            assert "context" in update_request.parameters["enhanced_parameters"]["input_schema"]["additional_fields"]
            
            # Test serialization
            serialized = update_request.model_dump(exclude_none=True)
            assert serialized["parameters"]["new_capabilities"]["sentiment_analysis"] is True
            assert serialized["parameters"]["enhanced_parameters"]["input_schema"]["additional_fields"] == ["context", "user_preferences"]
    ...
    def test_update_datapoint_request_with_complex_changes(self):
        ...
        try:
            ...
            # Verify complex nested structures
            assert update_request.inputs["user_preferences"]["location"] == "Paris, France"
            assert update_request.inputs["user_preferences"]["include_forecast"] is True
            assert update_request.metadata["quality_improvements"]["validation_status"] == "reviewed"
            
            # Test serialization
            serialized = update_request.model_dump(exclude_none=True)
            assert serialized["inputs"]["user_preferences"]["location"] == "Paris, France"
            assert serialized["metadata"]["quality_improvements"]["validation_status"] == "reviewed"
    ...
    def test_event_filter_with_complex_queries(self):
        ...
        try:
            ...
            complex_filters = [
                ...
                EventFilter(field="inputs.metadata.task_type", value="information_extraction"),
                EventFilter(field="metadata.session_context.conversation_id", value="conv-789")
            ]
            ...
            # Test serialization
            serialized_filters = [filter_obj.model_dump(exclude_none=True) for filter_obj in complex_filters]
    ...
    def test_update_project_request_with_complex_changes(self):
        ...
        try:
            ...
            assert update_request.description == "Enhanced project with new capabilities and metadata"
            
            # Test serialization
            serialized = update_request.model_dump(exclude_none=True)
            assert serialized["project_id"] == "project-123"
            assert serialized["name"] == "updated-complex-project"
            assert serialized["description"] == "Enhanced project with new capabilities and metadata"
    ...
    def test_update_run_request_with_complex_changes(self):
        ...
        try:
            ...
            # Verify complex nested structures
            assert update_request.metadata["changes"]["new_metrics"] == ["f1_score", "auc_roc"]
            assert update_request.metadata["changes"]["updated_thresholds"]["accuracy"] == 0.85
            assert update_request.metadata["changes"]["quality_improvements"]["enhanced_guidelines"] is True
            
            # Test serialization
            serialized = update_request.model_dump(exclude_none=True)
            assert serialized["metadata"]["changes"]["new_metrics"] == ["f1_score", "auc_roc"]
            assert serialized["metadata"]["changes"]["updated_thresholds"]["accuracy"] == 0.85
    ...
    def test_put_configuration_request_with_complex_changes(self):
        ...
        try:
            ...
            from honeyhive.models.generated import Parameters1, CallType, FunctionCallParams, EnvEnum, Type6
            ...
            assert update_request.parameters.hyperparameters["advanced"]["anthropic_specific"]["system_prompt"] == "You are a helpful AI assistant"
            assert update_request.type == Type6.LLM
            assert "Switched from OpenAI to Anthropic" in update_request.user_properties["change_log"]
    ...
    def test_create_dataset_request_with_complex_metadata(self):
        ...
        try:
            ...
            from honeyhive.models.generated import Type4, PipelineType
            ...
            assert dataset_request.metadata["quality_assurance"]["quality_score"] == 0.96
            assert "technology" in dataset_request.metadata["quality_assurance"]["coverage_metrics"]["domains"]
            assert dataset_request.metadata["quality_assurance"]["validation_process"]["automated_checks"] is True
            
            # Test serialization
            serialized = dataset_request.model_dump(exclude_none=True)
            assert serialized["metadata"]["quality_assurance"]["quality_score"] == 0.96
            assert "technology" in serialized["metadata"]["quality_assurance"]["coverage_metrics"]["domains"]
    ...
    def test_dataset_update_with_nested_changes(self):
        ...
        try:
            from honeyhive.models.generated import DatasetUpdate, Type4
                ...
                metadata={
                    ...
                    "changes": {
                        ...
                        "quality_improvements": {
                            ...
                            "validation_enhancements": ["automated_checks", "human_review"]
                        }
                    }
                }
            )
            ...
            assert update_request.metadata["changes"]["quality_improvements"]["improved_guidelines"] is True
            assert "automated_checks" in update_request.metadata["changes"]["quality_improvements"]["validation_enhancements"]
    ...
    def test_metric_edit_with_complex_changes(self):
        ...
        try:
            ...
            assert update_request.criteria == "Enhanced evaluation criteria with new dimensions and weights"
    ...
    def test_create_run_response_parsing(self):
        ...
        try:
            from honeyhive.models.generated import CreateRunResponse, UUIDType, Status
    ...
    def test_get_runs_response_parsing(self):
        ...
        try:
            from honeyhive.models.generated import GetRunsResponse, UUIDType, Status
    ...
    def test_get_run_response_parsing(self):
        ...
        try:
            from honeyhive.models.generated import GetRunResponse, UUIDType, Status
