...
class TestHoneyHiveTracer:
    ...
    def test_session_name_generation(self) -> None:
        ...
        with patch('honeyhive.tracer.otel_tracer.OTEL_AVAILABLE', True):
            with patch('honeyhive.tracer.otel_tracer.config') as mock_config:
                with patch('honeyhive.tracer.otel_tracer.time.time', return_value=1234567890):
                    mock_config.api_key = "test_key"
                    mock_config.project = "test_project"
                    
                    tracer = HoneyHiveTracer(api_key="test_key", project="test_project")
                    
                    # Should generate session name with timestamp
                    assert tracer.session_name == "tracer_session_1234567890"
    ...
    def test_instrumentors_integration(self) -> None:
        ...
        with patch('honeyhive.tracer.otel_tracer.OTEL_AVAILABLE', True):
            with patch('honeyhive.tracer.otel_tracer.config') as mock_config:
                with patch.object(HoneyHiveTracer, '_integrate_instrumentors') as mock_integrate:
                    mock_config.api_key = "test_key"
                    mock_config.project = "test_project"
    ...
    def test_initialization_flow(self) -> None:
        ...
        with patch('honeyhive.tracer.otel_tracer.OTEL_AVAILABLE', True):
            with patch('honeyhive.tracer.otel_tracer.config') as mock_config:
                with patch.object(HoneyHiveTracer, '_initialize_otel') as mock_init_otel:
                    with patch.object(HoneyHiveTracer, '_initialize_session') as mock_init_session:
                        with patch.object(HoneyHiveTracer, '_setup_baggage_context') as mock_setup_baggage:
                            mock_config.api_key = "test_key"
                            mock_config.project = "test_project"
    ...
    def test_initialization_error_handling(self) -> None:
        ...
        with patch('honeyhive.tracer.otel_tracer.OTEL_AVAILABLE', True):
            with patch('honeyhive.tracer.otel_tracer.config') as mock_config:
                ...
                with pytest.raises(ValueError):
                    HoneyHiveTracer(api_key=None, project="test_project", test_mode=False)
        ...
        with patch('honeyhive.tracer.otel_tracer.OTEL_AVAILABLE', True):
            with patch('honeyhive.tracer.otel_tracer.config') as mock_config:
                ...
                def create_tracer(thread_id: int) -> None:
                    try:
                        tracer = HoneyHiveTracer(api_key=f"key_{thread_id}", project=f"project_{thread_id}")
                ...
                tracers = [result[1] for result in results if not isinstance(result[1], Exception)]
