...
class TestProjectDeprecationIntegration:
    ...
    def test_export_without_project_works(
...
class TestEnrichSpanEventIdIntegration:
    ...
    def test_enrich_span_with_event_id_updates_event(
        ...
        # Find the event we created. `response.events` is List[LegacyEvent];
        # `_id` is an MongoDB-style alias that isn't declared on LegacyEvent,
        # but `extra="allow"` means it's still reachable via ``getattr``.
        for event in response.events:
            if event.event_name == "operation_to_enrich":
                event_id = event.event_id or getattr(event, "_id", None)
                break
        ...
        if event_id:
            ...
            for event in updated_response.events:
                if (event.event_id or getattr(event, "_id", None)) == event_id:
                    enriched_event = event
                    break

            if enriched_event:
                # Verify the enrichment was applied. ``metadata`` on
                # LegacyEvent is ``Optional[Dict[str, Any]]``.
                metadata = enriched_event.metadata or {}
