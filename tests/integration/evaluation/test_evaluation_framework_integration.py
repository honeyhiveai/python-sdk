...
class TestEvaluationFrameworkIntegration:
    ...
    def test_basic_evaluation_integration(self):
        ...
        assert "char_count" in result.metrics  # Length evaluator adds char_count directly
    ...
    def test_custom_evaluator_integration(self):
        ...
        assert result.metrics["test_custom"]["similarity_score"] > 0.8  # Custom logic should give high score
    ...
    def test_batch_evaluation_integration(self):
        ...
        dataset = [
            ...
            {"inputs": {"prompt": "Python"}, "outputs": {"response": "Programming language"}},
            {"inputs": {"prompt": "AI"}, "outputs": {"response": "Artificial Intelligence"}},
        ]
    ...
    def test_mixed_evaluator_types_integration(self):
        ...
        result = evaluate_with_evaluators(
            ...
            inputs={"prompt": "Hello"},
            outputs={"response": "Hi there!"},
            ground_truth={"expected": "Hi there!"},  # Add ground truth for custom evaluator
            max_workers=2
        )
...
class TestEvaluationAPIIntegration:
    ...
    def test_create_evaluation_run_integration(self, integration_client):
        """Test evaluation run creation integration."""
        # Mock the API response
        with patch.object(integration_client.evaluations, "create_run") as mock_create_run:
            # Create a mock response object that matches the expected structure
            mock_response = Mock()
            mock_evaluation = Mock()
            mock_evaluation.run_id = "test-run-123"
            mock_evaluation.name = "integration-test-run"
            mock_evaluation.project = "test-project"
            mock_evaluation.status = "completed"
            mock_response.evaluation = mock_evaluation
            
            mock_create_run.return_value = mock_response
    ...
    def test_evaluation_workflow_end_to_end(self, integration_client):
        ...
        with patch.object(integration_client.evaluations, "create_run") as mock_create_run:
            ...
            mock_response = Mock()
            mock_evaluation = Mock()
            mock_evaluation.run_id = "workflow-run-456"
            mock_evaluation.name = "end-to-end-test"
            mock_evaluation.project = "test-project"
            mock_evaluation.status = "completed"
            mock_response.evaluation = mock_evaluation
            ...
            dataset = [
                {"inputs": {"prompt": "What is 2+2?"}, "outputs": {"response": "The answer is 4"}},
                {"inputs": {"prompt": "What is 3+3?"}, "outputs": {"response": "The answer is 6"}},
                {"inputs": {"prompt": "What is 4+4?"}, "outputs": {"response": "The answer is 8"}},
            ]
            ...
            assert run is not None
            assert run.run_id == "workflow-run-456"
    
    def test_large_dataset_api_integration(self, integration_client):
        """Test large dataset evaluation with API integration."""
        # Mock API responses
        with patch.object(integration_client.evaluations, "create_run") as mock_create_run:
            # Create a mock response object
            mock_response = Mock()
            mock_evaluation = Mock()
            mock_evaluation.run_id = "large-dataset-run-789"
            mock_evaluation.name = "large-dataset-test"
            mock_evaluation.project = "test-project"
            mock_evaluation.status = "completed"
            mock_response.evaluation = mock_evaluation
            ...
            dataset_size = 50
            dataset = [
                {"inputs": {"prompt": f"Question {i}"}, "outputs": {"response": f"Answer {i}"}}
                for i in range(dataset_size)
            ]
            ...
            run = create_evaluation_run(
                ...
                project="test-project",
                results=results,
                metadata={"dataset_size": dataset_size, "evaluation_time": evaluation_time},
                client=integration_client
            )
            
            assert run is not None
            assert run.run_id == "large-dataset-run-789"
...
class TestEvaluationPerformanceIntegration:
    ...
    def test_concurrent_evaluation_integration(self):
        ...
        def evaluate_thread(thread_id: int):
            ...
            dataset = [
                {"inputs": {"prompt": f"T{thread_id}-Q{i}"}, "outputs": {"response": f"T{thread_id}-A{i}"}}
