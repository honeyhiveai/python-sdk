...
Tests Azure OpenAI integration with HoneyHive using both the Traceloop
(opentelemetry-instrumentation-openai) and OpenInference
(openinference-instrumentation-openai) instrumentors.

Requirements:
    pip install honeyhive opentelemetry-instrumentation-openai openinference-instrumentation-openai openai
...
pytestmark = [
    ...
    pytest.mark.skipif(
        not os.getenv("AZURE_OPENAI_ENDPOINT"), reason="AZURE_OPENAI_ENDPOINT not set"
    ),
    pytest.mark.slow,
]
    ...
    def test_chat_with_enrichment(self):
        ...
        try:
            ...
            def process_with_azure(prompt: str) -> str:
                enrich_span(
                    metadata={"deployment": os.getenv("AZURE_OPENAI_DEPLOYMENT")}
                )
...
class TestOpenInferenceAzureOpenAIIntegration:
    """Test Azure OpenAI integration via OpenInference instrumentor."""
    ...
    def test_basic_chat_completion(self):
        """Test basic Azure OpenAI chat completion is traced via OpenInference.
        ...
        tracer = HoneyHiveTracer.init(
            project=os.getenv("HH_PROJECT", "azure-openai-integration-test"),
            session_name="test_openinference_basic_chat_completion",
            source="pytest",
        )
    ...
    def test_chat_with_enrichment(self):
        """Test Azure OpenAI with span enrichment via OpenInference.
        ...
        tracer = HoneyHiveTracer.init(
            project=os.getenv("HH_PROJECT", "azure-openai-integration-test"),
            session_name="test_openinference_chat_with_enrichment",
            source="pytest",
        )
        ...
        try:
            ...
            result = process_with_azure(
                "Say 'azure openinference test' and nothing else."
            )
            assert result is not None
