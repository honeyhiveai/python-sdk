...
def generate_compatibility_matrix():
    ...
    lines.append("")
    lines.append(
        "*Generated on: {}*".format(datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    )
    lines.append("")
    lines.append(
    ...
    for category, providers in compatibility_data.items():
        ...
        for provider in providers:
            lines.append(
                f"| **{provider['provider']}** | {provider['models']} | `{provider['instrumentor']}` | {provider['integration']} | `{provider['test_file']}` | {provider['notes']} |"
            )
