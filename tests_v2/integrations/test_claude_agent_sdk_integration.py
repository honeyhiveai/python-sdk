...
class TestClaudeAgentSDKIntegration:
    ...
    async def test_basic_query_exports_session_events(self, fetch_events):
        ...
        try:
            ...
            async for _message in claude_agent_sdk.query(prompt=prompt, options=options):
                pass
        ...
        try:
            ...
            for event in events:
                ...
                if "tool" in event_type or "tool" in event_name:
                    if event.get("inputs") or event.get("outputs"):
            ...
            assert (
                ...
                f"{[(e.get('event_type'), e.get('event_name')) for e in events]}"
            )
        finally:
            instrumentor.uninstrument()
