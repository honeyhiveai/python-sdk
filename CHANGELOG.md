...
- **Tracing: Agent spans no longer mis-classified as model calls**
  - PydanticAI agent spans were incorrectly reported as `"model"` events instead of `"chain"` events
  - Agent spans are now correctly classified when they carry agent name attributes without an explicit request model
...
- **✨ API Enhancement: DatasetsAPI Filtering - Complete Backend Parity**
  ...
  - **Full Coverage**: SDK now exposes all backend filtering capabilities (`project`, `dataset_type`, `dataset_id`, `name`, `limit`)
