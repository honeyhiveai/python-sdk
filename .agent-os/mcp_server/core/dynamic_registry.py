...
from mcp_server.models.workflow import DynamicWorkflowContent, DynamicPhase
...
class DynamicContentRegistry:
    ...
    def __init__(
        ...
        try:
            ...
            raise DynamicRegistryError(f"Failed to load templates: {e}")
        
        # Parse source into structured phases
        try:
            phases = parser.parse(source_path)
        except Exception as e:
            raise DynamicRegistryError(f"Failed to parse source {source_path}: {e}")
    ...
    def _load_template(self, template_path: Path) -> str:
        ...
        try:
            ...
            raise DynamicRegistryError(f"Failed to read template {template_path}: {e}")
    ...
    def get_phase_metadata(self, phase: int) -> Dict[str, Any]:
        ...
        phase_data = next(
            (p for p in self.content.phases if p.phase_number == phase),
            None
        )
    ...
    def get_all_phases_metadata(self) -> list[Dict[str, Any]]:
        ...
        """
        return [
            self.get_phase_metadata(phase.phase_number)
