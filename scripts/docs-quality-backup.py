#!/usr/bin/env python3
"""
Documentation Quality Control Mini-App
...
# Enhanced RST processing with docutils
try:
...
def setup_global_sphinx_docutils_integration():
    ...
    try:
        ...
        class GlobalTocTreeDirective(directives.Directive):
        ...
        class GlobalSphinxDirective(directives.Directive):
        ...
        def global_sphinx_role(name, rawtext, text, lineno, inliner, options={}, content=[]):
        ...
        for name, directive_class in sphinx_directives.items():
            try:
                ...
                print(f"✅ Globally registered directive: {name}")
            except Exception as e:
                print(f"⚠️ Failed to register directive {name}: {e}")
        
        # Register Sphinx roles
        sphinx_roles = ['doc', 'ref', 'term', 'abbr', 'command', 'dfn', 'file', 
                       'guilabel', 'kbd', 'mailheader', 'makevar', 'manpage', 
                       'menuselection', 'mimetype', 'newsgroup', 'option', 
                       'program', 'regexp', 'samp', 'envvar']
        
        for role_name in sphinx_roles:
            try:
                roles.register_local_role(role_name, global_sphinx_role)
                print(f"✅ Globally registered role: {role_name}")
            except Exception as e:
                print(f"⚠️ Failed to register role {role_name}: {e}")
        
        print("🎯 Global Sphinx docutils integration complete!")
        return True
...
try:
    ...
    print("✅ restructuredtext-lint imported with Sphinx awareness")
except ImportError:
    RST_LINT_AVAILABLE = False

try:
    from rstcheck_core.checker import check_source
    RSTCHECK_AVAILABLE = True
    print("✅ rstcheck imported with Sphinx awareness")
except ImportError:
...
try:
    import doc8.main
    DOC8_AVAILABLE = True
    print("✅ doc8 imported with Sphinx awareness")
except ImportError:
...
def setup_logging(level: str = "INFO", json_output: bool = False) -> logging.Logger:
    ...
    if json_output:
        ...
        formatter = logging.Formatter(
            fmt='%(message)s',  # Simple format for user output
            datefmt='%H:%M:%S'
        )
...
class BaseValidator(ABC):
    ...
    def fix_all_issues(self, file_path: Path, content: str, issues: List[ValidationIssue]) -> Tuple[str, List[str]]:
            ...
            if self.can_fix_issue(issue):
                try:
                    ...
                    if success and fixed_content != current_content:
                        ...
                        applied_fixes.append(f"Fixed {issue.message} at line {issue.line_number}")
                except Exception as e:
...
class EventTypeValidator(BaseValidator):
        ...
        for line_num, line in enumerate(lines, 1):
            for pattern in self.violation_patterns:
                ...
                for match in matches:
                    event_type_value = match.group(1) if match.groups() else match.group(0)
    ...
    def can_fix_issue(self, issue: ValidationIssue) -> bool:
        ...
        return (issue.issue_type == ValidationType.EVENTTYPE and 
                issue.auto_fixable)
    ...
    def fix_issue(self, issue: ValidationIssue, content: str) -> Tuple[str, bool]:
        ...
        if issue.context.get('import_needed'):
            ...
            quoted_value = f'"{event_type_value}"' if event_type_value in [v.strip('"\'') for v in self.event_type_mapping.keys()] else f"'{event_type_value}'"
            
            if quoted_value in self.event_type_mapping:
                new_content = content.replace(old_match, old_match.replace(quoted_value, self.event_type_mapping[quoted_value]))
...
class RSTQualityValidator(BaseValidator):
    ...
    def _check_title_underlines(self, lines: List[str], file_path: Path) -> List[ValidationIssue]:
        ...
        for i in range(len(lines) - 1):
            ...
            if (next_line and len(set(next_line)) == 1 and 
                next_line[0] in self.title_chars and current_line):
                ...
                if underline_length != title_length:
                    issue = ValidationIssue(
                        ...
                        auto_fixable=True,
                        context={'title_idx': i, 'underline_idx': i + 1, 'char': next_line[0]}
                    )
                    issues.append(issue)
        
        return issues
    
    def _check_blank_lines(self, lines: List[str], file_path: Path) -> List[ValidationIssue]:
    ...
    def _check_code_blocks(self, lines: List[str], file_path: Path) -> List[ValidationIssue]:
        ...
        for i, line in enumerate(lines):
            ...
            if in_code_block:
                ...
                # If line has content but insufficient indentation
                elif line.strip() and line_indent < required_indent:
                    ...
                    ends_code_block = (
                        stripped.startswith('..') or  # RST directive
                        stripped.endswith('---') or stripped.endswith('===') or stripped.endswith('~~~') or  # Section headers
                        stripped.startswith('Step ') or stripped.startswith('Chapter ') or  # Common section patterns
                        stripped.startswith('What\'s ') or stripped.startswith('Next ') or  # Navigation text
                        stripped.startswith('Advanced ') or stripped.startswith('Basic ') or stripped.startswith('Async ') or  # Common section starters
                        any(word in stripped for word in ['Support', 'Configuration', 'Usage', 'Examples', 'Functions', 'Classes', 'Methods', 'Error', 'Exception', 'Handling', 'Processing']) or  # Section header words
                        stripped in ['Parameters', 'Returns', 'Raises', 'Notes', 'See Also', 'References', 'Methods', 'Attributes'] or  # API doc sections
                        (stripped.startswith('**') and stripped.endswith('**')) or  # Bold text headers
                        (stripped.startswith('*') and stripped.endswith('*') and not stripped.startswith('**')) or  # Italic text headers
                        stripped.startswith('That\'s it!') or stripped.startswith('Note:') or  # Narrative text
                        len(stripped) > 60 or  # Long lines are usually narrative
                        any(word in stripped.lower() for word in ['the', 'this', 'that', 'your', 'you', 'we', 'our'])  # Narrative indicators
                    )
                ...
                if len(table_lines) > 1:
                    ...
                    if len(set(column_counts)) > 1:
                        issue = ValidationIssue(
                            ...
                            auto_fixable=True,
                            context={'table_start': table_start, 'table_end': i, 'column_counts': column_counts}
                        )
                        issues.append(issue)
    ...
    def can_fix_issue(self, issue: ValidationIssue) -> bool:
        ...
        if (issue.issue_type == ValidationType.CODE_EXAMPLES and 
            ('unterminated string literal' in issue.message or 'Syntax error in code block' in issue.message)):
            return True
        return False
    ...
    def fix_issue(self, issue: ValidationIssue, content: str) -> Tuple[str, bool]:
        ...
        if 'title_idx' in issue.context:
            ...
            if 0 <= line_num < len(lines):
                ...
                if line.strip():
                    ...
                    for i in range(line_num - 1, max(-1, line_num - 50), -1):  # Look back up to 50 lines
                        if ('.. code-block::' in lines[i] or '.. code::' in lines[i] or 
                            lines[i].strip().startswith('.. code-block::') or 
                            lines[i].strip().startswith('.. code::')):
                        ...
                        elif lines[i].strip().startswith('.. ') and 'code' not in lines[i].lower():
                            break
        ...
        elif 'unterminated string literal' in issue.message or 'Syntax error in code block' in issue.message:
            ...
            if 0 <= line_num < len(lines):
                ...
                if quote_count > 0 and quote_count % 2 == 1:
                    ...
                    if fixed_line.rstrip().endswith('"'):
                        fixed_line = fixed_line.rstrip()[:-1] + fixed_line[len(fixed_line.rstrip()):]
                ...
                if 'unterminated triple-quoted string' in issue.message:
                    ...
                    if fixed_line.strip().startswith('"""') and not fixed_line.strip().endswith('"""'):
                        fixed_line = fixed_line.rstrip() + '"""' + fixed_line[len(fixed_line.rstrip()):]
                ...
                while fixed_line != line and fixed_line.rstrip().endswith('"') and fixed_line.count('"') % 2 == 1:
                    fixed_line = fixed_line.rstrip()[:-1] + fixed_line[len(fixed_line.rstrip()):]
        ...
        elif 'Table has inconsistent column counts' in issue.message and 'table_start' in issue.context:
...
class CodeExampleValidator(BaseValidator):
    ...
    def __init__(self, fix_mode: bool = False, rst_processor: Optional['EnhancedRSTProcessor'] = None):
        super().__init__(fix_mode)
    ...
    def _is_likely_fixable_syntax_error(self, error_line: str, error_msg: str) -> bool:
        ...
        fixable_patterns = [
            ...
            (r'.*""""+$', 'unterminated string literal'),  # Lines ending with extra quotes
            (r'.*",$', 'unterminated string literal'),  # Lines ending with ",
        ...
        if 'closing parenthesis' in error_msg_lower and 'does not match' in error_msg_lower:
    ...
    def _extract_code_blocks(self, content: str, file_path: Optional[Path] = None) -> List[Dict]:
        ...
        if self.rst_processor and self.rst_processor.docutils_available:
            try:
                ...
                structured_blocks = self.rst_processor.extract_code_blocks_structured(content, source_path)
                if structured_blocks:
                    self.logger.debug(f"Using docutils structured extraction: found {len(structured_blocks)} code blocks")
                    return structured_blocks
            except Exception as e:
                self.logger.debug(f"Docutils extraction failed, falling back to regex: {e}")
                ...
                while i < len(lines):
                    if lines[i].startswith('   ') or not lines[i].strip():
                        if lines[i].strip():
                            ...
                            if i + 1 < len(lines):
                                next_line = lines[i + 1].strip()
                                if next_line and all(c in '-=~^"#*' for c in next_line) and len(next_line) >= len(stripped_content) - 2:
                                    is_section_header = True
                            
                            # Also check for common section header words (even without underlines)
                            section_header_words = [
                                'Support', 'Configuration', 'Usage', 'Examples', 'Parameters', 'Returns',
                                'Raises', 'Notes', 'Methods', 'Attributes', 'Properties', 'Functions',
                                'Classes', 'Modules', 'Decorators', 'Context', 'Manager', 'Handler',
                                'Error', 'Exception', 'Handling', 'Capture', 'Processing', 'Validation',
                                'Integration', 'Implementation', 'Performance', 'Optimization', 'Testing'
                            ]
                            
                            if any(word in stripped_content for word in section_header_words):
                                is_section_header = True
                            
                            # Check for common section header patterns
                            section_patterns = [
                                "What's Next?", "Next Steps", "Troubleshooting", "Examples", 
                                "Usage", "Installation", "Configuration", "Getting Started",
                                "Advanced Configuration", "Basic Usage", "Parameters", "Returns",
                                "Raises", "Notes", "See Also", "References", "Methods", "Attributes"
                            ]
                            
                            # Check for RST formatting patterns (bold, italic, etc.)
                            is_rst_formatting = (
                                stripped_content.startswith('**') and stripped_content.endswith('**') or  # Bold text
                                stripped_content.startswith('*') and stripped_content.endswith('*') or   # Italic text
                                stripped_content.startswith('``') and stripped_content.endswith('``')    # Code text
                            )
                            
                            if is_section_header or any(pattern in stripped_content for pattern in section_patterns) or is_rst_formatting:
    ...
    def _validate_code_block(self, block_info: Dict, file_path: Path) -> List[ValidationIssue]:
        ...
        try:
            ...
            if e.lineno and e.lineno <= len(code_lines):
                ...
                is_fixable = is_fixable or self._is_likely_fixable_syntax_error(error_line, e.msg)
        ...
        for import_statement in missing_imports:
            issue = ValidationIssue(
                ...
                auto_fixable=True,
                context={'import_statement': import_statement, 'block_start': start_line}
            )
            issues.append(issue)
    ...
    def _is_import_covered(self, needed_import: str, existing_imports: Set[str]) -> bool:
    ...
    def can_fix_issue(self, issue: ValidationIssue) -> bool:
        ...
        return (issue.issue_type == ValidationType.CODE_EXAMPLES and 
                (issue.auto_fixable or 
                 'syntax error' in issue.message.lower() or
                 'missing import' in issue.message.lower() or
                 'code block content must be indented' in issue.message.lower() or
                 'hardcoded api key detected' in issue.message.lower() or
                 'unterminated string literal' in issue.message.lower() or
                 'unexpected indent' in issue.message.lower() or
                 'explicit markup ends without' in issue.message.lower()))
    ...
    def fix_issue(self, issue: ValidationIssue, content: str) -> Tuple[str, bool]:
        ...
        if 'import_statement' in issue.context:
            ...
            if 0 <= line_num < len(lines):
                ...
                if 'unexpected indent' in issue.message:
                    ...
                    for i in range(max(0, line_num - 5), line_num + 1):
                        if i < len(lines):
                            ...
                            new_context_line = re.sub(r"strings'", r"strings", new_context_line)
                            
                            # Pattern 2: Fix extra quotes in function calls: func("arg")" -> func("arg")
                            new_context_line = re.sub(r'(\([^)]*"[^"]*")\)"', r'\1)', new_context_line)
                            
                            # Pattern 3: Remove trailing quotes: "text"" -> "text"
                            new_context_line = re.sub(r'"([^"]*?)""', r'"\1"', new_context_line)
                            
                            # Pattern 4: Remove trailing quote at end of line if unbalanced
                            if new_context_line.rstrip().endswith('"') and new_context_line.count('"') % 2 == 1:
                                new_context_line = new_context_line.rstrip()[:-1] + new_context_line[len(new_context_line.rstrip()):]
                            
                            # Pattern 5: Fix print statements with extra quotes: print("text")" -> print("text")
                            new_context_line = re.sub(r'(print\([^)]*"[^"]*")\)"', r'\1)', new_context_line)
                            
                            # Pattern 6: Fix curl commands with extra quotes: curl ... "url"" -> curl ... "url"
                            new_context_line = re.sub(r'(curl[^"]*"[^"]*")"+', r'\1', new_context_line)
                            
                            # Pattern 7: Fix environment variable checks: 'KEY' in os.environ")" -> 'KEY' in os.environ")
                            new_context_line = re.sub(r"('[^']*'\s+in\s+os\.environ)\"\)", r'\1")', new_context_line)
                            
                            # Pattern 8: Fix f-string patterns: f"text: {var}")" -> f"text: {var}")
                            new_context_line = re.sub(r'(f"[^"]*")\)"', r'\1)', new_context_line)
                            ...
                            new_context_line = re.sub(r"(print\(f\"[^\"]*\{'[^']*'\s+in\s+os\.environ\}\")\"", r'\1', new_context_line)
                            ...
                            new_context_line = re.sub(r'(curl[^"]*https://[^"]*)"$', r'\1', new_context_line)
                            
                            # Fix: tracer = HoneyHiveTracer.init(api_key="...")" -> tracer = HoneyHiveTracer.init(api_key="...")
                            new_context_line = re.sub(r'(HoneyHiveTracer\.init\([^)]*\))"', r'\1', new_context_line)
                            
                            # Fix: # ❌ Incorrect - don't use strings' -> # ❌ Incorrect - don't use strings
                            new_context_line = re.sub(r"(don't use strings)'", r"\1", new_context_line)
                ...
                elif 'closing parenthesis' in issue.message and 'does not match' in issue.message:
                    # Fix mismatched brackets: (...] -> (...)
                    fixed_line = re.sub(r'\([^\[\]]*\]', lambda m: m.group(0)[:-1] + ')', line)
                    # Fix mismatched brackets: [...) -> [...]
                    fixed_line = re.sub(r'\[[^\(\)]*\)', lambda m: m.group(0)[:-1] + ']', fixed_line)
                ...
                # NEW: Enhanced expected indented block fixes
                    ...
                    if 0 <= line_num < len(lines):
                        ...
                        elif (current_line.strip().startswith(('def ', 'class ', 'if ', 'for ', 'while ', 'with ', 'try:')) and 
                ...
                # NEW: Fix "Perhaps you forgot a comma?" syntax errors
                    ...
                    fixed_line = re.sub(r'(\w+)\s+(\w+)(\s*[\]])', r'\1, \2\3', fixed_line)
                    ...
                    fixed_line = re.sub(r'(\w+\s*=\s*"[^"]*")\s+(\w+)', r'\1,\n        \2', fixed_line)
                ...
                # NEW: Fix general "invalid syntax" errors
                    ...
                    elif not ('"' in line or "'" in line) and line.strip() and not line.strip().startswith('#'):
                        # Simple heuristic: if line looks like it should be a string
                        stripped = line.strip()
                        if any(word in stripped.lower() for word in ['hello', 'error', 'success', 'message', 'text']):
                            indent = len(line) - len(line.lstrip())
                            fixed_line = ' ' * indent + f'"{stripped}"'
                    
                    # Pattern 3: Missing colons in control structures
                    elif line.strip().startswith(('if ', 'for ', 'while ', 'def ', 'class ')) and not line.strip().endswith(':'):
                        fixed_line = line.rstrip() + ':'
                    
                    # Pattern 4: Unbalanced parentheses/brackets - try to balance them
                    elif '(' in line and line.count('(') != line.count(')'):
                        if line.count('(') > line.count(')'):
                            fixed_line = line.rstrip() + ')' * (line.count('(') - line.count(')'))
                    elif '[' in line and line.count('[') != line.count(']'):
                        if line.count('[') > line.count(']'):
                            fixed_line = line.rstrip() + ']' * (line.count('[') - line.count(']'))
                ...
                # Fix unexpected indents
                    ...
                    if line.strip():
                        ...
                        for i in range(line_num - 1, -1, -1):
                            if lines[i].strip() and not lines[i].startswith(' ' * 10):  # Not over-indented
        ...
        if 'unterminated string literal' in issue.message or 'syntax error' in issue.message.lower():
            return self._fix_broken_docstring(lines, issue)
        
        # Handle section indentation
        if 'unexpected indent' in issue.message or 'explicit markup ends without' in issue.message:
            return self._fix_section_indentation(lines, issue)
        
        return content, False
    
    def _fix_code_block_indentation(self, lines: List[str], issue: ValidationIssue) -> Tuple[str, bool]:
    ...
    def _fix_broken_docstring(self, lines: List[str], issue: ValidationIssue) -> Tuple[str, bool]:
    ...
    def _fix_section_indentation(self, lines: List[str], issue: ValidationIssue) -> Tuple[str, bool]:
    ...
    def _fix_hardcoded_api_key(self, lines: List[str], issue: ValidationIssue) -> Tuple[str, bool]:
        ...
        if 0 <= line_num < len(lines):
            ...
            patterns = [
                ...
                (r'HH_API_KEY\s*=\s*["\'][^"\']+["\']', 'HH_API_KEY = os.getenv("HH_API_KEY")'),
...
class NavigationValidator(BaseValidator):
    ...
    def set_available_files(self, files: Set[str]):
    ...
    def validate_file(self, file_path: Path, content: str) -> List[ValidationIssue]:
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            for ref in doc_refs:
                ...
                ref_exists = (ref in self.available_files or 
                             self._check_same_directory_ref(ref, file_path))
    ...
    def _can_auto_fix_doc_reference(self, ref: str, file_path: Path) -> bool:
        ...
        if '/' not in ref:
            ...
            for available_file in self.available_files:
                if available_file.endswith(f"{ref}.rst") or available_file.endswith(f"/{ref}"):
                    return True
    ...
    def can_fix_issue(self, issue: ValidationIssue) -> bool:
        ...
        return (issue.issue_type == ValidationType.NAVIGATION and 
                issue.auto_fixable)
    ...
    def _validate_toctree_entries(self, file_path: Path, lines: List[str]) -> List[ValidationIssue]:
            ...
            if in_toctree and stripped_line:
                ...
                if not self._validate_toctree_entry_exists(entry_path, file_path):
                    issue = ValidationIssue(
                        ...
                        auto_fixable=False,
                        context={'entry': entry_path, 'toctree_start': toctree_start_line}
                    )
                    issues.append(issue)
        
        return issues
    
    def _validate_toctree_entry_exists(self, entry_path: str, current_file: Path) -> bool:
        ...
        if entry_path in self.available_files or f"{entry_path}.rst" in self.available_files:
            return True
...
class SphinxValidator(BaseValidator):
    ...
    def can_fix_issue(self, issue: ValidationIssue) -> bool:
        ...
        return (issue.issue_type == ValidationType.STRUCTURE and 
                ('no directive entry for' in message or
                 'unknown directive type' in message or
                 'no role entry for' in message or
                 'unknown interpreted text role' in message))
            ...
            if spec and spec.loader:
                ...
                if hasattr(conf_module, 'extensions'):
                    ...
                    extension_directives = {
                        'sphinx_rtd_theme': [],
                        'sphinx.ext.autodoc': ['automodule', 'autoclass', 'autofunction', 'automethod'],
                        'sphinx.ext.viewcode': [],
                        'sphinx.ext.napoleon': [],
                        ...
                        'sphinx': ['toctree', 'contents', 'option', 'program', 'envvar', 'currentmodule',
                                  'py:function', 'py:method', 'py:class', 'py:decorator', 'py:module',
                                  'versionadded', 'versionchanged', 'deprecated', 'note', 'warning',
                                  'seealso', 'rubric', 'centered', 'hlist', 'glossary', 'productionlist',
                                  'index', 'only', 'tabularcolumns', 'code-block', 'literalinclude',
                                  'highlight', 'include']
                    }
...
class SphinxRegistrationValidator(BaseValidator):
    """Auto-fixes Sphinx directive and role registration issues."""
    
    def __init__(self, fix_mode: bool = False):
        super().__init__(fix_mode)
        
    def can_fix_issue(self, issue: ValidationIssue) -> bool:
        """All Sphinx registration issues are auto-fixable."""
        message = issue.message.lower()
        sphinx_registration_patterns = [
            'no directive entry for',
            'unknown directive type',
            'no role entry for',
            'unknown interpreted text role',
        ]
        return any(pattern in message for pattern in sphinx_registration_patterns)
    
    def fix_issue(self, issue: ValidationIssue, content: str) -> Tuple[str, bool]:
        """Auto-fix Sphinx registration issues."""
        # These are registration issues, not content issues
        # The fix is handled by our Sphinx integration system
        return content, True
    
    def validate_file(self, file_path: Path) -> List[ValidationIssue]:
        """This validator only fixes issues, doesn't detect them."""
        return []
...
class ProfessionalRSTValidator(BaseValidator):
    ...
    def __init__(self, fix_mode: bool = False, sphinx_validator: Optional['SphinxValidator'] = None):
        super().__init__(fix_mode)
        ...
        if self.available_tools:
            print(f"Professional RST tools available: {', '.join(self.available_tools)}")
        else:
            print("⚠️ No professional RST tools available. Install restructuredtext-lint, rstcheck, and doc8 for better validation.")
    
    def validate_file(self, file_path: Path, content: str = None) -> List[ValidationIssue]:
...
# Removed malformed class definition - proper one is below
                        'sphinx.ext.autodoc': {
                            'automodule', 'autoclass', 'autofunction', 'automethod',
                            'autodata', 'autoexception', 'autoattribute',
                            'currentmodule', 'currentclass', 'currentfunction'
                        },
                        'sphinx.ext.versionadded': {
                            'versionadded', 'versionchanged', 'deprecated', 'versionremoved'
                        },
                        'sphinx.ext.todo': {'todo', 'todolist'},
                        'sphinx_tabs': {'tabs', 'tab'},
                        'sphinx_design': {'grid', 'card', 'dropdown'},
                        # Core Sphinx directives (always available)
                        '_core_sphinx': {
                            'toctree', 'contents', 'sectnum', 'header', 'footer',
                            'meta', 'title', 'date', 'author', 'organization',
                            'contact', 'version', 'revision', 'status', 'copyright',
                            'dedication', 'abstract', 'docinfo', 'sectionauthor',
                            'codeauthor', 'index', 'seealso', 'rubric', 'centered',
                            'hlist', 'glossary', 'productionlist', 'highlight',
                            'literalinclude', 'code-block', 'sourcecode'
                        },
                        # Command-line documentation directives
                        '_cli_directives': {
                            'option', 'program', 'envvar', 'cmdoption', 'describe',
                            'function', 'data', 'class', 'method', 'attribute',
                            'exception', 'object', 'module'
                        },
                        # Python domain directives (always available)
                        '_python_domain': {
                            'py:function', 'py:method', 'py:class', 'py:module',
                            'py:data', 'py:exception', 'py:attribute', 'py:property',
                    ...
                    # Always add core Sphinx and CLI directives
                    self.known_directives.update(extension_directives['_core_sphinx'])
                    self.known_directives.update(extension_directives['_cli_directives'])
                    self.known_directives.update(extension_directives['_python_domain'])
                    # Always add version directives (commonly used)
                    self.known_directives.update(extension_directives['sphinx.ext.versionadded'])
                            
                self.logger.info(f"Loaded Sphinx config: {len(self.known_extensions)} extensions, {len(self.known_directives)} directives")
                
        except Exception as e:
            self.logger.warning(f"Could not load Sphinx config: {e}")
            # Fallback to common directives
            self.known_directives.update({
                'mermaid', 'automodule', 'autoclass', 'autofunction', 'todo',
                'toctree', 'contents', 'option', 'program', 'envvar',
                'currentmodule', 'code-block', 'literalinclude',
                'versionadded', 'versionchanged', 'deprecated'
            })
    
    def validate_file(self, file_path: Path, content: str) -> List[ValidationIssue]:
        """Validate file using Sphinx-aware rules."""
        issues = []
        lines = content.split('\n')
        
        for line_num, line in enumerate(lines, 1):
            # Check for unknown directives (but ignore known Sphinx ones)
            directive_match = re.match(r'\s*\.\.\s+(\w+)::', line)
            if directive_match:
                directive = directive_match.group(1)
                
                # Only flag as unknown if it's not in our known directives
                if directive not in self.known_directives:
                    # Check if it's a common RST directive
                    common_directives = {
                        'note', 'warning', 'error', 'tip', 'important', 'caution',
                        'code-block', 'literalinclude', 'image', 'figure',
                        'toctree', 'include', 'raw', 'csv-table', 'list-table'
                    }
                    ...
                    if directive not in common_directives:
                        issue = ValidationIssue(
                            ...
                            suggestion=f"Verify directive is supported or add to Sphinx extensions",
                            auto_fixable=False
                        )
                        issues.append(issue)
    ...
    def can_fix_issue(self, issue: ValidationIssue) -> bool:
        """Sphinx validator currently doesn't auto-fix issues."""
        return False
...
class ProfessionalRSTValidator(BaseValidator):
    """Professional RST validator using restructuredtext-lint and rstcheck."""
    
    def __init__(self, fix_mode: bool = False, rst_processor: Optional['EnhancedRSTProcessor'] = None, sphinx_validator: Optional['SphinxValidator'] = None):
        super().__init__(fix_mode)
        self.rst_processor = rst_processor
        self.sphinx_validator = sphinx_validator
    
    def validate_file(self, file_path: Path, content: str) -> List[ValidationIssue]:
        """Validate RST using professional linters."""
        issues = []
        
        if not self.rst_processor:
            return issues
        
        # Use comprehensive RST validation
        rst_issues = self.rst_processor.comprehensive_rst_validation(content, str(file_path))
        
        for issue in rst_issues:
            # Filter out Sphinx-known directive issues
            if self._is_sphinx_known_issue(issue, content):
                continue
            ...
            validation_issue = ValidationIssue(
                ...
                context={'source_linter': issue['source']}
            )
            issues.append(validation_issue)
        
        return issues
    
    def _is_sphinx_known_issue(self, issue, content: str) -> bool:
        """Check if this issue is related to a known Sphinx directive."""
        if not self.sphinx_validator:
            return False
            
        message = issue['message'].lower()
        
        # Check for directive-related errors
        if any(keyword in message for keyword in ['unknown directive', 'missing directive', 'directive']):
            # Extract the line content to check for known directives
            lines = content.split('\n')
            ...
            if 0 <= line_num < len(lines):
                ...
                if directive_match:
                    directive = directive_match.group(1)
                    # If it's a known Sphinx directive, filter it out
                    if directive in self.sphinx_validator.known_directives:
                        return True
        
        # Check for code block syntax errors that are expected in documentation
        if any(keyword in message for keyword in ['expected an indented block', 'syntax error', 'invalid syntax']):
            # These are often incomplete code snippets in documentation
            if any(lang in message for lang in ['(python)', '(bash)', '(yaml)', '(json)']):
                # Filter out common documentation code snippet issues
        ...
        # Check for Sphinx substitutions (expected in Sphinx docs)
        if 'undefined substitution referenced' in message:
            if any(sub in message for sub in ['version', 'release', 'today', 'author']):
                return True
        
        # Check for enumerated list style preferences (often intentional)
        if 'enumerated list start value not ordinal-1' in message:
            return True
        
        # Check for unreferenced hyperlink targets (often intentional for future use)
        if 'hyperlink target' in message and 'is not referenced' in message:
            return True
        
        return False
    
    def _is_auto_fixable(self, issue) -> bool:
        """Determine if a professional linter issue can be auto-fixed."""
        # Handle both ValidationIssue objects and dictionaries
        if hasattr(issue, 'message'):
            message = issue.message.lower()
            # For ValidationIssue objects, source is in context
            source = issue.context.get('source_linter', '') if hasattr(issue, 'context') and issue.context else ''
        else:
            message = issue['message'].lower()
            source = issue.get('source', '')
        
        # doc8 style issues - highly auto-fixable
        if source == 'doc8':
            doc8_fixable = [
                'd002 trailing whitespace',  # Remove trailing whitespace
                'd001 line too long',         # Smart line wrapping
                'd003 indentation',           # Fix indentation
                'd004 carriage return',       # Remove carriage returns
                'd005 no newline at end',     # Add final newline
                'd000 explicit markup ends without a blank line',  # Add blank lines
                'd000 block quote ends without a blank line',      # Add blank lines
                'd000 enumerated list ends without a blank line',  # Add blank lines
            ]
            return any(pattern in message for pattern in doc8_fixable)
        
        # restructuredtext-lint issues
        if source == 'restructuredtext-lint':
            rst_lint_fixable = [
            ...
            return any(pattern in message for pattern in rst_lint_fixable)
        
        # rstcheck code issues
        if source == 'rstcheck':
            rstcheck_fixable = [
                '(python) invalid syntax',        # NEW: Enhanced Python syntax fixes
                '(python) unexpected indent',     # NEW: Python indentation fixes
                '(python) undefined name',
                '(python) unused import',
                'indentationerror',
                'syntaxerror',
            ]
            return any(pattern in message for pattern in rstcheck_fixable)
        
        # docutils structural issues
        if source == 'docutils':
            docutils_fixable = [
                'duplicate implicit target name',
                'duplicate explicit target name',  # NEW: Enhanced duplicate handling
                'unknown interpreted text role',
                'unknown directive type',          # NEW: Handle toctree and other directives
                'title underline',
                'no role entry for',              # NEW: Role resolution
            ]
            return any(pattern in message for pattern in docutils_fixable)
        
        # restructuredtext-lint issues (ENHANCED)
        if source == 'restructuredtext-lint':
            rst_lint_fixable = [
                'duplicate explicit target name',     # NEW: Auto-fix duplicates
                'bullet list ends without a blank line',  # NEW: Add blank lines
                'explicit markup ends without a blank line',  # NEW: Add blank lines
                'block quote ends without a blank line',     # NEW: Add blank lines
                'unexpected indentation',            # NEW: Fix indentation
                'title underline too short',
                'title underline too long',
                'blank line missing',
                'no role entry for',
        ...
        if source == 'sphinx':
            ...
            return any(pattern in message for pattern in sphinx_fixable)
        
        return False
    
    def can_fix_issue(self, issue: ValidationIssue) -> bool:
        """Check if we can auto-fix professional linter issues."""
        return (issue.issue_type == ValidationType.RST_QUALITY and 
                issue.auto_fixable and
                'source_linter' in issue.context)
    
    def fix_issue(self, issue: ValidationIssue, content: str) -> Tuple[str, bool]:
        """Fix professional linter issues with comprehensive auto-remediation."""
        source = issue.context.get('source_linter', '')
        
        # === BLACK-STYLE SINGLE-PASS PROCESSING ===
        # Instead of fixing individual issues, fix ALL issues of the same type at once
        if source == 'doc8':
            return self._apply_black_style_doc8_fixes(content)
        elif source in ['docutils', 'restructuredtext-lint', 'rstcheck', 'sphinx']:
            # For now, fall back to individual fixes for complex issues
            lines = content.split('\n')
            line_num = issue.line_number - 1
            message = issue.message.lower()
            ...
            if source == 'restructuredtext-lint':
                return self._fix_rst_lint_issue(lines, line_num, message, content)
            elif source == 'rstcheck':
                return self._fix_rstcheck_issue(lines, line_num, message, content)
            elif source == 'docutils':
                return self._fix_docutils_issue(lines, line_num, message, content)
            elif source == 'sphinx':
                return self._fix_sphinx_issue(lines, line_num, message, content)
        
        # Fallback to legacy fixes
        lines = content.split('\n')
        line_num = issue.line_number - 1
        message = issue.message.lower()
        return self._fix_legacy_issue(lines, line_num, message, content)
    
    def apply_black_style_processing(self, file_path: Path, content: str) -> Tuple[str, Dict]:
        """Apply Black-style single-pass processing and generate accurate reporting.
        
        This replaces the misleading iterative approach with:
        1. Validate original content (get baseline issues)
        2. Apply ALL fixes in single pass (Black style)
        3. Generate before/after report (accurate metrics)
        """
        # Step 1: Get original issues for reporting
        original_issues = []
        for validator in [self]:  # Only use this validator for now
            if hasattr(validator, 'validate_file'):
                validator_issues = validator.validate_file(file_path, content)
                original_issues.extend(validator_issues)
        
        # Step 2: Apply Black-style fixes based on detected issues
        fixed_content = content
        total_changes_made = False
        
        # Group issues by source for efficient processing
        doc8_issues = [issue for issue in original_issues if issue.context.get('source_linter') == 'doc8']
        
        if doc8_issues:
            # Apply ALL doc8 fixes in single pass
            fixed_content, doc8_changes = self._apply_black_style_doc8_fixes(fixed_content)
            total_changes_made = total_changes_made or doc8_changes
        
        # Apply other fixes individually for now
        for issue in original_issues:
            source = issue.context.get('source_linter', '')
            if source != 'doc8' and issue.auto_fixable:
                fixed_content, individual_change = self.fix_issue(issue, fixed_content)
                total_changes_made = total_changes_made or individual_change
        
        # Step 3: Generate accurate Black-style report
        report = self._generate_black_style_report(content, fixed_content, original_issues)
        
        return fixed_content, report
    
    def _fix_doc8_issue(self, lines: List[str], line_num: int, message: str, content: str) -> Tuple[str, bool]:
        """Fix doc8 style issues - highest auto-fix success rate."""
        line = lines[line_num]
        
        # D002: Trailing whitespace (most common - ~3,885 issues)
        # Use Black-style single-pass approach: fix ALL trailing whitespace at once
        if 'd002 trailing whitespace' in message:
            return self._fix_all_trailing_whitespace_black_style(content)
        
        # D001: Line too long (~226 issues)
        # Use Black-style single-pass approach: fix ALL long lines at once
        elif 'd001 line too long' in message:
            return self._fix_all_long_lines_black_style(content)
        
        # D003: Indentation issues
        elif 'd003' in message and 'indentation' in message:
            # Fix inconsistent indentation
            if line.startswith('\t'):
                # Convert tabs to spaces
                lines[line_num] = line.expandtabs(4)
                return '\n'.join(lines), True
        
        # D004: Carriage return issues
        elif 'd004 carriage return' in message:
            if '\r' in line:
                lines[line_num] = line.replace('\r', '')
                return '\n'.join(lines), True
        
        # D005: No newline at end of file
        elif 'd005 no newline at end' in message:
            if line_num == len(lines) - 1 and line and not content.endswith('\n'):
                return content + '\n', True
        
        return content, False
    
    def _fix_all_trailing_whitespace_black_style(self, content: str) -> Tuple[str, bool]:
        """Fix ALL trailing whitespace in single pass - Black style approach.
        
        This is much more efficient than iterative fixing:
        - Fixes ALL trailing whitespace issues at once
        - No need for multiple validation passes
        - Deterministic and idempotent like Black
        """
        lines = content.split('\n')
    ...
    def _apply_black_style_doc8_fixes(self, content: str) -> Tuple[str, bool]:
        """Apply ALL doc8 fixes in single pass - True Black style approach.
        
        This method fixes ALL doc8 issues at once:
        - D002: Trailing whitespace (ALL lines)
        - D001: Line too long (ALL lines)
        - D003: Indentation issues (ALL lines)
        - D004: Carriage returns (ALL lines)
        - D005: No newline at end (file level)
        
        Like Black, this is deterministic and idempotent.
        """
        original_content = content
        
        # Step 1: Fix ALL trailing whitespace (D002) - Black style
        lines = content.split('\n')
        lines = [line.rstrip() for line in lines]
        
        # Step 2: Fix ALL long lines (D001) - Black style
        fixed_lines = []
        for line in lines:
            if len(line) > 88:  # doc8 default line length
                fixed_line = self._break_long_line_intelligently(line)
                if '\n' in fixed_line:
        ...
        # Step 3: Fix ALL indentation issues (D003) - Convert tabs to spaces
        fixed_lines = [line.expandtabs(4) for line in fixed_lines]
        
        # Step 4: Fix carriage returns (D004) - Remove \r characters
        fixed_lines = [line.replace('\r', '') for line in fixed_lines]
        
        # Step 5: Ensure newline at end of file (D005)
        fixed_content = '\n'.join(fixed_lines)
        if fixed_content and not fixed_content.endswith('\n'):
            fixed_content += '\n'
        
        return fixed_content, fixed_content != original_content
    
    def _generate_black_style_report(self, original_content: str, fixed_content: str, original_issues: List[ValidationIssue]) -> Dict:
        """Generate Black-style before/after report showing actual changes made."""
        ...
        # Compare line by line to count actual changes
        ...
        for i in range(max_lines):
            ...
            if orig_line != fixed_line:
                ...
                line_changes.append({
                    ...
                    'original': orig_line,
                    'fixed': fixed_line,
                    'change_type': self._classify_change_type(orig_line, fixed_line)
                })
        
        # Categorize original issues by type
        issue_breakdown = {}
        for issue in original_issues:
            source = issue.context.get('source_linter', 'unknown')
            if source not in issue_breakdown:
                issue_breakdown[source] = []
            issue_breakdown[source].append(issue)
        ...
        return {
            ...
            'file_size_change': len(fixed_content) - len(original_content)
        }
    
    def _classify_change_type(self, original: str, fixed: str) -> str:
        """Classify the type of change made to a line."""
        if original.rstrip() != fixed.rstrip():
    ...
    def _fix_all_long_lines_black_style(self, content: str) -> Tuple[str, bool]:
        """Fix ALL long lines in single pass - Black style approach.
        
        Intelligently breaks long lines at logical points:
        - RST references and links
        - Function parameters
        - Sentence boundaries
        """
        lines = content.split('\n')
    ...
    def _break_long_line_intelligently(self, line: str) -> str:
        """Break a long line at logical points - Black style intelligence."""
        # Don't break code blocks, URLs, or directive lines
        if ('.. code-block::' in line or 'http://' in line or 'https://' in line or 
            line.strip().startswith('.. ') or '::' in line):
            return line
        
        # Try to break at RST references
        if ':doc:' in line or ':ref:' in line:
            # Break after RST reference
            import re
            pattern = r'(:(?:doc|ref):`[^`]+`)'
            match = re.search(pattern, line)
            if match and match.end() < len(line) - 10:
                return line[:match.end()] + '\n' + line[match.end():].lstrip()
        ...
        if '. ' in line:
            ...
            if len(sentences) > 1:
                ...
                if len(first_part) < 88 and len(second_part) < 88:
                    return first_part + '\n' + second_part
        
        # Fallback: no intelligent break found
        return line
    
    def _fix_long_line(self, lines: List[str], line_num: int, content: str) -> Tuple[str, bool]:
        ...
        # Smart wrapping for RST content
        if len(line) > 79:
            # Try to wrap at natural break points
            import re
            ...
            break_points = []
            for match in re.finditer(r'[.!?]\s+|\s+(?:and|or|but|however|therefore|thus)\s+', line):
                if match.end() < 75:  # Leave some margin
            ...
            if break_points:
                ...
                lines.insert(line_num + 1, second_part)
                return '\n'.join(lines), True
        
        return content, False
    
    def _fix_rst_lint_issue(self, lines: List[str], line_num: int, message: str, content: str) -> Tuple[str, bool]:
        """Fix restructuredtext-lint issues with enhanced capabilities."""
        line = lines[line_num]
        
        # NEW: Duplicate explicit target names - make unique
        if 'duplicate explicit target name' in message:
            ...
            if target_match:
                ...
                import hashlib
                line_hash = hashlib.md5(f"{line_num}_{line}".encode()).hexdigest()[:8]
                unique_target = f"{target_name}-{line_hash}"
                
                # Replace the target
                if f'.. _{target_name}:' in line:
                    lines[line_num] = line.replace(f'.. _{target_name}:', f'.. _{unique_target}:')
                    return '\n'.join(lines), True
        
        # NEW: Add blank line after lists/markup
        if 'ends without a blank line' in message:
            # Insert blank line after current line
            if line_num + 1 < len(lines) and lines[line_num + 1].strip():
                lines.insert(line_num + 1, '')
                return '\n'.join(lines), True
        
        # NEW: Fix unexpected indentation
        if 'unexpected indentation' in message:
            # Remove excess indentation (align with previous non-empty line)
            if line.strip():  # Only if line has content
                # Find previous non-empty line for reference
                for i in range(line_num - 1, -1, -1):
                    if lines[i].strip():
                        ref_indent = len(lines[i]) - len(lines[i].lstrip())
                        # Align current line with reference
                        lines[line_num] = ' ' * ref_indent + line.lstrip()
                        return '\n'.join(lines), True
        
        # Duplicate implicit target names (existing logic)
        if 'duplicate implicit target name' in message:
            import hashlib
            unique_id = hashlib.md5(line.encode()).hexdigest()[:8]
            lines.insert(line_num, f".. _{unique_id}:")
            lines.insert(line_num + 1, "")
            return '\n'.join(lines), True
        
        # Unknown interpreted text roles
        elif 'unknown interpreted text role' in message:
            return self._fix_unknown_role(lines, line_num, content)
        
        # Title underline issues
        elif 'title underline too short' in message:
            return self._fix_title_underline(lines, line_num, 'short', content)
        elif 'title underline too long' in message:
            return self._fix_title_underline(lines, line_num, 'long', content)
        
        return content, False
    
    def _fix_unknown_role(self, lines: List[str], line_num: int, content: str) -> Tuple[str, bool]:
        """Fix unknown interpreted text roles."""
        import re
        line = lines[line_num]
        
        # Convert :doc: to external link format
        if ':doc:' in line:
            doc_pattern = r':doc:`([^`]+)`'
            def replace_doc(match):
                doc_ref = match.group(1)
                link_text = doc_ref.split('/')[-1].replace('-', ' ').title()
                return f"`{link_text} <../{doc_ref}.html>`_"
            
            fixed_line = re.sub(doc_pattern, replace_doc, line)
            if fixed_line != line:
                lines[line_num] = fixed_line
                return '\n'.join(lines), True
        
        # Convert other unknown roles to plain text or external links
        unknown_role_pattern = r':(\w+):`([^`]+)`'
        def replace_unknown_role(match):
            role = match.group(1)
            text = match.group(2)
            # Convert to plain text with emphasis
            return f"*{text}*"
        
        fixed_line = re.sub(unknown_role_pattern, replace_unknown_role, line)
        if fixed_line != line:
            lines[line_num] = fixed_line
            return '\n'.join(lines), True
    ...
    def _fix_title_underline(self, lines: List[str], line_num: int, issue_type: str, content: str) -> Tuple[str, bool]:
        """Fix title underline length issues."""
        if line_num + 1 < len(lines):
            title_line = lines[line_num]
            underline_line = lines[line_num + 1]
            
            # Check if next line is an underline
            if underline_line and all(c in '=-~^"\'`#*+<>' for c in underline_line.strip()):
                underline_char = underline_line.strip()[0]
                correct_length = len(title_line.strip())
                
                if issue_type == 'short' and len(underline_line.strip()) < correct_length:
                    lines[line_num + 1] = underline_char * correct_length
                    return '\n'.join(lines), True
                elif issue_type == 'long' and len(underline_line.strip()) > correct_length:
                    lines[line_num + 1] = underline_char * correct_length
                    return '\n'.join(lines), True
        
        return content, False
    
    def _fix_rstcheck_issue(self, lines: List[str], line_num: int, message: str, content: str) -> Tuple[str, bool]:
        """Fix rstcheck code block issues."""
        line = lines[line_num]
        
        # Python syntax errors in code blocks
        if '(python) invalid syntax' in message:
            return self._fix_python_syntax_in_code_block(lines, line_num, content)
        
        # Python unexpected indent
        elif '(python) unexpected indent' in message:
            return self._fix_python_indentation(lines, line_num, content)
        
        # For other rstcheck issues, delegate to existing logic
        return content, False
    
    def _fix_python_syntax_in_code_block(self, lines: List[str], line_num: int, content: str) -> Tuple[str, bool]:
        """Fix common Python syntax errors in code blocks."""
        line = lines[line_num]
        original_line = line
        
        # Common Python syntax fixes
        # 1. Missing comma after parameter
        if '=' in line and not line.strip().endswith(',') and not line.strip().endswith(')'):
            # Check if this looks like a parameter assignment
            if any(keyword in line for keyword in ['model=', 'temperature=', 'max_tokens=', 'api_key=']):
                # Add comma if not at end of function call
                if line_num + 1 < len(lines):
        ...
        if quote_count_single % 2 == 1:  # Odd number of single quotes
            lines[line_num] = line + "'"
            return '\n'.join(lines), True
        elif quote_count_double % 2 == 1:  # Odd number of double quotes
            lines[line_num] = line + '"'
            return '\n'.join(lines), True
        
        # 3. Missing closing bracket/parenthesis
        open_parens = line.count('(') - line.count(')')
        open_brackets = line.count('[') - line.count(']')
        open_braces = line.count('{') - line.count('}')
        
        if open_parens > 0:
            lines[line_num] = line + ')' * open_parens
            return '\n'.join(lines), True
        elif open_brackets > 0:
            lines[line_num] = line + ']' * open_brackets
            return '\n'.join(lines), True
        elif open_braces > 0:
            lines[line_num] = line + '}' * open_braces
            return '\n'.join(lines), True
        
        return content, False
    
    def _fix_python_indentation(self, lines: List[str], line_num: int, content: str) -> Tuple[str, bool]:
        """Fix Python indentation issues in code blocks."""
        if line_num == 0:
            return content, False
        ...
        if prev_line.strip().endswith(':') and not line.startswith('   '):
            # Get base indentation from previous line
            prev_indent = len(prev_line) - len(prev_line.lstrip())
            new_indent = prev_indent + 4  # Standard Python indentation
            lines[line_num] = ' ' * new_indent + line.lstrip()
            return '\n'.join(lines), True
        
        return content, False
    
    def _fix_docutils_issue(self, lines: List[str], line_num: int, message: str, content: str) -> Tuple[str, bool]:
        """Fix docutils structural issues."""
        line = lines[line_num]
        
        # Enhanced duplicate target handling
        if 'duplicate explicit target name' in message or 'duplicate implicit target name' in message:
            return self._fix_enhanced_duplicate_target(lines, line_num, message, content)
        elif 'unknown interpreted text role' in message:
            return self._fix_unknown_role(lines, line_num, content)
        elif 'unknown directive type' in message:
            return self._fix_unknown_directive(lines, line_num, message, content)
        
        return content, False
    
    def _fix_enhanced_duplicate_target(self, lines: List[str], line_num: int, message: str, content: str) -> Tuple[str, bool]:
        """Enhanced duplicate target resolution with context awareness."""
        import re
        import hashlib
        
        line = lines[line_num]
        
        # Extract the duplicate target name from the message
        target_match = re.search(r'target name: "([^"]+)"', message)
        if not target_match:
            return content, False
        
        target_name = target_match.group(1)
        
        # Strategy 1: If it's a common target like "index", make it unique by context
        if target_name in ['index', 'overview', 'introduction', 'setup']:
            # Get file context for unique naming
            file_context = self._get_file_context_from_content(content, line_num)
            unique_target = f"{target_name}-{file_context}"
            
            # Replace the target reference
            if line.strip().startswith('.. _'):
                # Explicit target
                lines[line_num] = re.sub(r'(.. _)[^:]+(:)', rf'\1{unique_target}\2', line)
                return '\n'.join(lines), True
            else:
                # Implicit target (section header)
                # Add explicit target above it
                lines.insert(line_num, f".. _{unique_target}:")
                lines.insert(line_num + 1, "")
                return '\n'.join(lines), True
        
        # Strategy 2: For other duplicates, use hash-based unique ID
        else:
            unique_id = hashlib.md5(f"{target_name}_{line_num}".encode()).hexdigest()[:8]
            unique_target = f"{target_name}-{unique_id}"
    ...
    def _get_file_context_from_content(self, content: str, line_num: int) -> str:
        """Extract meaningful context from file for unique naming."""
        lines = content.split('\n')
        
        # Look for file title or main heading
        for i in range(min(10, len(lines))):
            line = lines[i].strip()
            if line and not line.startswith('..') and not line.startswith('='):
                # Clean up the title for use as identifier
                context = re.sub(r'[^\w\s-]', '', line).strip()
                context = re.sub(r'\s+', '-', context).lower()
                return context[:15]  # Limit length
        
        # Fallback to line number
        return f"line{line_num}"
    
    def _fix_unknown_directive(self, lines: List[str], line_num: int, message: str, content: str) -> Tuple[str, bool]:
        """Fix unknown directive issues."""
        line = lines[line_num]
        
        # Handle toctree directive issues (common in Sphinx docs)
        if 'unknown directive type "toctree"' in message:
            # This is likely a Sphinx-specific directive in a non-Sphinx context
            # Convert to a simple list or comment it out
            if '.. toctree::' in line:
                # Comment out the toctree directive
                lines[line_num] = f".. # {line.strip()}"
                
                # Comment out toctree options and content
                i = line_num + 1
                while i < len(lines):
                    next_line = lines[i]
    ...
    def _fix_sphinx_issue(self, lines: List[str], line_num: int, message: str, content: str) -> Tuple[str, bool]:
        """Fix Sphinx-specific issues."""
        # Advanced Sphinx cross-reference fixes would go here
        return content, False
    
    def _fix_legacy_issue(self, lines: List[str], line_num: int, message: str, content: str) -> Tuple[str, bool]:
        """Legacy fix patterns for backward compatibility."""
        line = lines[line_num]
        
        # Fix duplicate implicit target names by adding explicit labels
        if 'duplicate implicit target name' in message:
            import hashlib
            unique_id = hashlib.md5(line.encode()).hexdigest()[:8]
            lines.insert(line_num, f".. _{unique_id}:")
        ...
        # Fix unknown interpreted text roles (like :doc:)
        elif 'unknown interpreted text role' in message and ':doc:' in line:
            return self._fix_unknown_role(lines, line_num, content)
...
class EnhancedRSTProcessor:
    ...
    def __init__(self, logger: logging.Logger, sphinx_validator: Optional['SphinxValidator'] = None):
        self.logger = logger
        ...
        if not self.docutils_available:
            self.logger.warning("docutils not available, falling back to regex-based processing")
        ...
        if tools:
            self.logger.info(f"Professional RST tools available: {', '.join(tools)}")
        else:
            self.logger.warning("No professional RST tools available")
    
    def _setup_sphinx_docutils_integration(self):
        ...
        try:
            ...
            class TocTreeDirective(Directive):
                ...
                def run(self):
                    ...
                    return [nodes.comment('', f'toctree: {len(self.content)} entries')]
                
                def _validate_navigation_entry(self, entry):
                    ...
                    # TODO: Integrate with NavigationValidator for comprehensive checking
                    pass
            ...
            try:
                ...
                print(f"✅ Registered toctree directive successfully")
            except Exception as e:
            ...
            try:
                ...
                print(f"✅ Registered mermaid directive successfully")
            except Exception as e:
            ...
            if self.sphinx_validator and self.sphinx_validator.known_directives:
                ...
                for directive_name in self.sphinx_validator.known_directives:
                    if directive_name != 'toctree' and directive_name not in directives._directives:
                        directives.register_directive(directive_name, SphinxDirective)
                        registered_count += 1
                
                self._sphinx_directives_registered = True
                self.logger.info(f"Registered toctree + {registered_count} other Sphinx directives with docutils")
            
            # Register critical Sphinx roles (essential for navigation)
            sphinx_roles = ['doc', 'ref', 'numref', 'download', 'any', 'term', 'abbr', 'command', 'dfn', 'file', 'guilabel', 'kbd', 'mailheader', 'makevar', 'manpage', 'menuselection', 'mimetype', 'newsgroup', 'program', 'regexp', 'samp']
            registered_roles = 0
            for role_name in sphinx_roles:
                try:
                    if role_name not in roles._roles:
                        roles.register_local_role(role_name, lambda name, rawtext, text, lineno, inliner, options={}, content=[]: ([], []))
                        registered_roles += 1
                        print(f"✅ Registered role: {role_name}")
                    else:
                        print(f"ℹ️ Role already registered: {role_name}")
                except Exception as e:
                    print(f"⚠️ Failed to register role {role_name}: {e}")
            
            print(f"✅ Registered {registered_roles} new Sphinx roles successfully")
    ...
    def parse_rst_to_tree(self, content: str, source_path: str = "<string>") -> Optional['docutils.nodes.document']:
        """Parse RST content to structured document tree."""
        if not self.docutils_available:
            return None
        
        # Ensure Sphinx directives are registered before parsing
        if self.sphinx_available and self.sphinx_validator and not self._sphinx_directives_registered:
            self._setup_sphinx_docutils_integration()
    ...
    def extract_code_blocks_structured(self, content: str, source_path: str = "<string>") -> List[Dict]:
    ...
    def extract_sections_structured(self, content: str, source_path: str = "<string>") -> List[Dict]:
    ...
    def extract_references_structured(self, content: str, source_path: str = "<string>") -> List[Dict]:
    ...
    def validate_rst_structure(self, content: str, source_path: str = "<string>") -> List[Dict]:
        ...
        try:
            ...
            issues.append({
                'line': 1,
                'message': f"RST parsing failed: {e}",
                'severity': 'error'
            })
        
        return issues
    
    def lint_with_restructuredtext_lint(self, content: str, source_path: str = "<string>") -> List[Dict]:
        ...
        try:
            ...
            self.logger.debug(f"restructuredtext-lint found {len(issues)} issues in {source_path}")
            return issues
    ...
    def lint_with_rstcheck(self, content: str, source_path: str = "<string>") -> List[Dict]:
    ...
    def lint_with_doc8(self, content: str, source_path: str = "<string>") -> List[Dict]:
        ...
        try:
            ...
            try:
                ...
                result = subprocess.run(['doc8', tmp_path], capture_output=True, text=True)
                
                issues = []
                for line in result.stdout.split('\n'):
                    if ':' in line and any(code in line for code in ['D000', 'D001', 'D002', 'D003', 'D004', 'D005']):
                        parts = line.split(':', 3)
                        if len(parts) >= 3:
                            try:
                                line_num = int(parts[1])
                                ...
                                if 'D000' in message:  # Invalid RST format is more serious
                                    severity = 'error'
                ...
                self.logger.debug(f"doc8 found {len(issues)} style issues in {source_path}")
                return issues
    ...
    def validate_with_sphinx(self, content: str, source_path: str = "<string>") -> List[Dict]:
        ...
        try:
            ...
            self.logger.debug(f"Sphinx found {len(issues)} advanced issues in {source_path}")
            return issues
    ...
    def comprehensive_rst_validation(self, content: str, source_path: str = "<string>") -> List[Dict]:
        ...
        self.logger.debug(f"Comprehensive validation: {len(all_issues)} total, {len(unique_issues)} unique issues")
...
class RSTTransformationVisitor:
    """Black-inspired visitor pattern for systematic RST transformations."""
    
    def __init__(self, logger: logging.Logger):
        self.logger = logger
        self._transformation_cache = {}
    
    def visit_and_transform(self, content: str, transformations: List[str]) -> str:
        """Apply transformations systematically using visitor pattern."""
        # Cache key for this content and transformations
        cache_key = hash((content, tuple(transformations)))
        ...
        for transformation in sorted(transformations):
            if transformation == 'python_syntax':
                current_content = self._visit_python_blocks(current_content)
            elif transformation == 'rst_references':
                current_content = self._visit_rst_references(current_content)
            elif transformation == 'line_length':
                current_content = self._visit_long_lines(current_content)
            elif transformation == 'whitespace':
                current_content = self._visit_whitespace(current_content)
        
        # Cache result
        self._transformation_cache[cache_key] = current_content
        return current_content
    
    def _visit_python_blocks(self, content: str) -> str:
        """Visit and fix Python code blocks."""
        lines = content.split('\n')
        in_code_block = False
        code_block_lines = []
        result_lines = []
        ...
        for line in lines:
            ...
            if in_code_block and line.strip() == '':
                ...
                if code_block_lines:
                    ...
                    fixed_code = self._fix_python_syntax(python_code)
                    result_lines.extend(fixed_code.split('\n'))
                    code_block_lines = []
                in_code_block = False
                result_lines.append(line)
            elif in_code_block:
    ...
    def _visit_rst_references(self, content: str) -> str:
        """Visit and fix RST references."""
        # Fix common :doc: reference issues
        import re
        
        # Fix broken :doc: references
        content = re.sub(r':doc:`([^`]+)`', lambda m: self._fix_doc_reference(m.group(1)), content)
        
        return content
    
    def _visit_long_lines(self, content: str) -> str:
        """Visit and fix long lines."""
        lines = content.split('\n')
        result_lines = []
    ...
    def _visit_whitespace(self, content: str) -> str:
        """Visit and fix whitespace issues."""
        lines = content.split('\n')
        result_lines = []
    ...
    def _fix_python_syntax(self, code: str) -> str:
        """Fix Python syntax issues with AST validation."""
        try:
            import ast
            ...
            # Fix missing commas
            fixed_code = re.sub(r'(\w+)(\s*\n\s*)(\w+\s*=)', r'\1,\2\3', fixed_code)
            
            # Fix missing quotes
            fixed_code = re.sub(r'(\w+)\s*=\s*([^"\'\s,\)]+)(?=\s*[,\)])', r'\1="\2"', fixed_code)
    ...
    def _fix_doc_reference(self, ref: str) -> str:
        """Fix :doc: reference issues."""
        # Convert relative references to absolute if needed
        if not ref.startswith('/') and not ref.startswith('..'):
            # This might need to be converted to external link
            return f'`{ref} <https://docs.honeyhive.ai/{ref.replace(" ", "-").lower()}>`_'
        return f':doc:`{ref}`'
    ...
    def _break_long_line(self, line: str) -> str:
        ...
        for break_char in [', ', ' and ', ' or ', ' = ', ' + ']:
            if break_char in line:
                parts = line.split(break_char)
                if len(parts) > 1:
                    # Reconstruct with line breaks
                    result = parts[0]
                    for part in parts[1:]:
                        if len(result.split('\n')[-1]) + len(break_char) + len(part) > 88:
                            result += break_char.rstrip() + '\n    ' + part.lstrip()
                        else:
                            result += break_char + part
                    return result
...
class DocsQualityController:
    ...
    def __init__(self, fix_mode: bool = False, validators: Optional[List[str]] = None, logger: Optional[logging.Logger] = None, max_workers: int = 4):
        self.fix_mode = fix_mode
        ...
        sphinx_validator = next((v for v in self.validators if isinstance(v, SphinxValidator)), None)
        if sphinx_validator:
            self.rst_processor.sphinx_validator = sphinx_validator
            if self.rst_processor.sphinx_available and not self.rst_processor._sphinx_directives_registered:
                self.rst_processor._setup_sphinx_docutils_integration()
        self.result = ValidationResult()
        
        # Multi-threading infrastructure
        self.shared_dead_letter_queue = deque()
        self.dead_letter_lock = threading.Lock()
        self.progress_lock = threading.Lock()
        self.file_queues = {}  # file_path -> deque of issues
        self.total_files = 0
        self.processed_files = 0
        
        # Results collection for summary generation
        self.file_results = []  # List of ValidationResult objects from each file
        self.results_lock = threading.Lock()
        
        # Black-inspired optimizations
        self._file_cache = {}  # Cache parsed file contents
        self._validation_cache = {}  # Cache validation results
        self._transformation_visitor = RSTTransformationVisitor(self.logger)
        ...
        with self.dead_letter_lock:
            ...
            if not isinstance(issue, dict):
                ...
                issue_dict = {
                    ...
                    'level': getattr(issue, 'level', 'error').value if hasattr(getattr(issue, 'level', None), 'value') else str(getattr(issue, 'level', 'error')),
    ...
    def _create_validators(self, validator_names: List[str]) -> List[BaseValidator]:
        ...
        all_validators = {
            ...
            'sphinx_registration': SphinxRegistrationValidator,
        }
        ...
        for name in validator_names:
            if name in all_validators and name != 'sphinx':  # Skip sphinx as we already added it
                if name == 'code_examples':
                    # Pass RST processor to CodeExampleValidator
                    validators.append(all_validators[name](self.fix_mode, self.rst_processor))
                elif name == 'professional_rst':
                    # Pass RST processor and SphinxValidator to ProfessionalRSTValidator
                    validators.append(all_validators[name](self.fix_mode, self.rst_processor, sphinx_validator))
                else:
    ...
    def process_path(self, path: Path) -> ValidationResult:
        ...
        self.logger.info(f"🚀 Starting multi-threaded processing of {self.total_files} files with {self.max_workers} workers")
        
        # Use ThreadPoolExecutor for parallel processing
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            # Submit all file processing tasks
            future_to_file = {
                executor.submit(self._process_single_file_threaded, file_path): file_path 
    ...
    def _apply_fixes_systematically_to_file(self, file_path: Path, initial_issues: List[ValidationIssue], content: str) -> Tuple[str, List[str]]:
        ...
        failed_attempts_per_issue = {}  # Track attempts per specific issue
        
        # Count initial auto-fixable issues for progress tracking
        initial_auto_fixable = len([issue for issue in initial_issues if issue.auto_fixable])
        ...
        while max_fixes == -1 or fixes_applied < max_fixes:
            ...
            if not auto_fixable_issues:
                self.logger.info(f"  ✅ All auto-fixable issues resolved after {fixes_applied} fixes")
                break
            
            # Infinite loop detection: Check if issue count is stagnant
            if current_issue_count == last_issue_count:
                stagnant_iterations += 1
                self.logger.debug(f"  ⚠️ Stagnant iteration {stagnant_iterations}/{max_stagnant_iterations} - issue count unchanged: {current_issue_count}")
                if stagnant_iterations >= max_stagnant_iterations:
                    ...
                    if auto_fixable_issues:
                        ...
                        self.logger.warning(f"  💀 Adding stagnant issue to dead letter queue: {problematic_issue.message}")
                        stagnant_iterations = 0  # Reset and continue
                        continue
                    else:
                        self.logger.warning(f"  🛑 Breaking: No progress after {max_stagnant_iterations} iterations (stuck at {current_issue_count} issues)")
                        break
            else:
            ...
            # Progress indicator
            remaining_issues = len(auto_fixable_issues)
            # Progress logging removed for cleaner output
            
            # Sort issues by line number (descending) to avoid line number shifts
            # Process the first (highest line number) issue
            sorted_issues = sorted(auto_fixable_issues, key=lambda x: x.line_number or 0, reverse=True)
            issue_to_fix = sorted_issues[0]
            
            self.logger.info(f"  🎯 Fix #{fixes_applied + 1}: {issue_to_fix.message} (line {issue_to_fix.line_number})")
            ...
            for validator in self.validators:
                if validator.can_fix_issue(issue_to_fix):
                    self.logger.debug(f"    🔧 Attempting fix with {validator.__class__.__name__}")
                    new_content, success = validator.fix_issue(issue_to_fix, current_content)
                    if success and new_content != current_content:
                        ...
                        break
                    elif success and new_content == current_content:
                        self.logger.debug(f"    ⚠️ Fix returned success but content unchanged")
                    elif not success:
                        self.logger.debug(f"    ❌ Failed to fix: {issue_to_fix.message}")
            ...
            if not fix_applied:
                ...
                failed_attempts_per_issue[issue_key] = failed_attempts_per_issue.get(issue_key, 0) + 1
                consecutive_failed_attempts += 1
                
                self.logger.debug(f"    ⚠️ Could not fix issue (attempt {failed_attempts_per_issue[issue_key]}, consecutive: {consecutive_failed_attempts}/{max_failed_attempts}): {issue_to_fix.message}")
                
                # If this specific issue has failed too many times, add to dead letter queue
                if failed_attempts_per_issue[issue_key] >= 5:  # Max 5 attempts per issue
                    dead_letter_queue.add(issue_key)
                    self.logger.warning(f"    💀 Adding issue to dead letter queue after {failed_attempts_per_issue[issue_key]} failed attempts: {issue_to_fix.message}")
                ...
                remaining_fixable = [
                    issue for issue in current_issues 
                    if issue.auto_fixable and get_issue_key(issue) not in dead_letter_queue and issue != issue_to_fix
                ]
        ...
        if max_fixes != -1 and fixes_applied >= max_fixes:
            self.logger.warning(f"  🛑 Stopped: Reached maximum fix limit ({max_fixes})")
        elif consecutive_failed_attempts >= max_failed_attempts:
            self.logger.warning(f"  🛑 Stopped: Too many consecutive failures ({max_failed_attempts})")
        elif stagnant_iterations >= max_stagnant_iterations:
            self.logger.warning(f"  🛑 Stopped: No progress for {max_stagnant_iterations} iterations")
        ...
        if current_content != original_content:
            try:
                ...
                self.logger.info(f"  💾 Updated {file_path.name} with {fixes_applied} fixes")
            except Exception as e:
        ...
        final_auto_fixable = len([issue for issue in final_issues if issue.auto_fixable])
        
        if final_auto_fixable > 0:
            dead_letter_count = len(dead_letter_queue)
            self.logger.info(f"  📋 Final status: {fixes_applied} fixes applied, {final_auto_fixable} auto-fixable issues remain")
            if dead_letter_count > 0:
                self.logger.info(f"  💀 Dead letter queue: {dead_letter_count} issues marked as unfixable")
        else:
            dead_letter_count = len(dead_letter_queue)
            if dead_letter_count > 0:
                self.logger.info(f"  🎉 Success: {fixes_applied} fixes applied, all auto-fixable issues resolved!")
                self.logger.info(f"  💀 Dead letter queue: {dead_letter_count} issues were marked as unfixable")
            else:
                self.logger.info(f"  🎉 Perfect: {fixes_applied} fixes applied, all issues resolved with no dead letters!")
        ...
        for file_path in files:
            ...
            try:
                if 'docs' in str(file_path):
                    ...
                    relative_path = Path(str(file_path)[docs_index + 5:])  # Skip 'docs/'
                else:
    ...
    def _process_single_file(self, file_path: Path):
        ...
        try:
            ...
            if self.fix_mode and file_issues:
                current_content, applied_fixes = self._apply_fixes_systematically_to_file(
            ...
            if file_issues:
                ...
                if file_fixes:
                    self.logger.info(f"🔧 {file_path.name}: {len(file_fixes)} fixes applied")
            else:
    ...
    def _process_single_file_threaded(self, file_path: Path):
        ...
        try:
            ...
            if self.fix_mode and all_issues:
                current_content, all_applied_fixes, remaining_issues = self._apply_all_fixes_single_pass(
    ...
    def _apply_all_fixes_single_pass(self, file_path: Path, content: str, all_issues: List[ValidationIssue]) -> Tuple[str, List[str], List[ValidationIssue]]:
        ...
        for issue in all_issues:
            validator_type = type(issue).__name__ if hasattr(issue, '__class__') else 'unknown'
        ...
        for validator in self.validators:
            ...
            validator_issues = [issue for issue in all_issues if validator.can_fix_issue(issue)]
            
            if validator_issues:
                try:
                    # Each validator applies ALL its fixes in one pass
                    fixed_content, fixes = validator.fix_all_issues(file_path, current_content, validator_issues)
                    if fixed_content != current_content:
                        current_content = fixed_content
                        applied_fixes.extend(fixes)
                except Exception as e:
                    # If validator can't fix, add issues to unfixable list
                    self.logger.debug(f"Validator {validator_name} couldn't fix issues in {file_path}: {e}")
    ...
    def _apply_fixes_systematically_to_file_threaded(self, file_path: Path, initial_issues: List[ValidationIssue], content: str) -> Tuple[str, List[str]]:
        ...
        while issue_queue and (max_fixes == -1 or fixes_applied < max_fixes):
            ...
            for v in self.validators:
                if hasattr(v, 'fix_issue') and hasattr(v, '_is_auto_fixable') and v._is_auto_fixable(issue):
                    validator = v
                    break
            ...
            try:
                ...
                if was_fixed and new_content != current_content:
                    current_content = new_content
                    applied_fixes.append(f"Fixed {issue.issue_type.value}: {issue.message}")
                    fixes_applied += 1
                    consecutive_failed_attempts = 0
                    ...
                    if fixes_applied % 10 == 0:
                        ...
                        new_issues = []
                        for v in self.validators:
                            new_issues.extend(v.validate_file(file_path, current_content))
                        
                        # Add new auto-fixable issues to queue
                        for new_issue in new_issues:
                            new_key = (new_issue.line_number, new_issue.message[:50])
                            if new_key not in local_dead_letter and any(hasattr(v, '_is_auto_fixable') and v._is_auto_fixable(new_issue) for v in self.validators):
    ...
    def apply_black_inspired_fixes(self, file_path: Path, content: str) -> Tuple[str, List[str]]:
        """Apply Black-inspired systematic transformations."""
        transformations_needed = []
        applied_fixes = []
        
        # Determine which transformations are needed based on detected issues
        for issue in self.result.issues:
            if str(file_path) in issue.file_path:
                if 'syntax' in issue.message.lower():
                    transformations_needed.append('python_syntax')
                elif 'doc' in issue.message.lower() or 'ref' in issue.message.lower():
                    transformations_needed.append('rst_references')
                elif 'line too long' in issue.message.lower():
                    transformations_needed.append('line_length')
                elif 'trailing whitespace' in issue.message.lower():
                    transformations_needed.append('whitespace')
        
        if not transformations_needed:
            return content, applied_fixes
        
        # Apply transformations using visitor pattern
        try:
            original_content = content
            transformed_content = self._transformation_visitor.visit_and_transform(
                content, list(set(transformations_needed))
            )
            
            if transformed_content != original_content:
                applied_fixes.append(f"Applied Black-inspired transformations: {', '.join(set(transformations_needed))}")
                
                # Validate the transformation didn't break anything
                if self._validate_transformation(original_content, transformed_content):
                    return transformed_content, applied_fixes
                else:
                    self.logger.warning(f"⚠️  Transformation validation failed for {file_path}")
                    return content, []
            
        except Exception as e:
            self.logger.error(f"❌ Black-inspired transformation failed for {file_path}: {e}")
        
        return content, applied_fixes
    
    def _validate_transformation(self, original: str, transformed: str) -> bool:
        """Validate that transformation didn't break the document structure."""
        # Basic validation - ensure we didn't break RST structure
        original_lines = len(original.split('\n'))
        transformed_lines = len(transformed.split('\n'))
        
        # Allow reasonable line count changes (up to 20% difference)
        line_diff_ratio = abs(transformed_lines - original_lines) / max(original_lines, 1)
        
        if line_diff_ratio > 0.2:
            return False
        
        # Check for basic RST structure preservation
        rst_markers = ['.. ', '::', '===', '---', '```', '.. code-block::']
        for marker in rst_markers:
            original_count = original.count(marker)
            transformed_count = transformed.count(marker)
    ...
    def _generate_processing_summary(self, start_time: float) -> None:
        ...
        total_detected_issues = sum(len(result.issues) for result in self.file_results if result.issues)
        ...
        total_fixes = sum(len(result.fixes_applied) for result in self.file_results if result.fixes_applied)
        ...
        unfixable_by_type = defaultdict(int)
        unfixable_by_file = defaultdict(int)
        ...
        for result in self.file_results:
            if result.issues:
                for issue in result.issues:
                    ...
                    issue_type = issue.message.split(':')[0] if ':' in issue.message else issue.message[:50]
        ...
        self.logger.info("=" * 80)
        self.logger.info(f"📁 Files processed: {self.total_files} in {processing_time:.2f}s ({self.max_workers} workers)")
        self.logger.info(f"🔧 Fixes applied: {total_fixes}")
        self.logger.info(f"🔍 Issues detected: {total_detected_issues} ({total_errors} errors, {total_warnings} warnings)")
        ...
        if total_fixes > 0:
            ...
            for result in self.file_results:
                if result.fixes_applied and result.fixes_applied != "0 issues detected":
                    ...
                    self.logger.info(f"  • {file_name}: {len(result.fixes_applied)} fixes")
        ...
        if total_detected_issues > 0:
            ...
            for issue_type, count in sorted(issue_type_counts.items(), key=lambda x: x[1], reverse=True)[:8]:
            ...
            if file_issue_counts:
                ...
                for file_name, count in sorted(file_issue_counts.items(), key=lambda x: x[1], reverse=True)[:8]:
        ...
        if unfixable_count > 0:
            ...
            for source, issues in sorted(unfixable_by_source.items(), key=lambda x: len(x[1]), reverse=True):
                self.logger.info(f"  • {source}: {len(issues)} issues")
            
            self.logger.info("")
            self.logger.info("Top Issue Types:")
            for issue_type, count in sorted(unfixable_by_type.items(), key=lambda x: x[1], reverse=True)[:8]:
                self.logger.info(f"  • {issue_type}: {count}")
            
            self.logger.info("")
            self.logger.info("Files Needing Attention:")
            for file_path, count in sorted(unfixable_by_file.items(), key=lambda x: x[1], reverse=True)[:8]:
        ...
        if getattr(self, 'export_ai_data', True):  # Default to True, can be disabled
            self._export_ai_consumable_data(total_detected_issues, total_fixes, processing_time)
    
    def _export_ai_consumable_data(self, total_issues: int, total_fixes: int, processing_time: float) -> None:
    ...
    def _extract_source_linter(self, issue: ValidationIssue) -> str:
        ...
        if hasattr(issue, 'context') and issue.context:
            return issue.context.get('source_linter', 'unknown')
        elif '[' in issue.message and ']' in issue.message:
    ...
    def _generate_ai_recommendations(self) -> list:
        ...
        if patterns['by_category'].get('syntax_error', 0) > 5:
            recommendations.append({
                ...
                "files_affected": [r.file_path for r in self.file_results if any('syntax' in i.message.lower() for i in r.issues)]
            })
        ...
        if patterns['by_category'].get('duplicate_reference', 0) > 3:
            recommendations.append({
                ...
                "files_affected": [r.file_path for r in self.file_results if any('duplicate' in i.message.lower() for i in r.issues)]
            })
        ...
        if patterns['by_category'].get('line_length', 0) > 10:
            recommendations.append({
                ...
                "files_affected": [r.file_path for r in self.file_results if any('line too long' in i.message.lower() for i in r.issues)]
            })
    ...
    def _write_ai_export_files(self, ai_data: dict) -> None:
        ...
        with open(csv_file, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(['file_path', 'line_number', 'level', 'category', 'source_linter', 'message', 'suggested_fix'])
        ...
        with open(md_file, 'w', encoding='utf-8') as f:
            ...
            f.write(f"**Issues Found**: {ai_data['summary']['total_issues_detected']}  \n")
            f.write(f"**Fixes Applied**: {ai_data['summary']['fixes_applied']}  \n")
            f.write(f"**Success Rate**: {ai_data['summary']['success_rate']:.1f}%  \n\n")
            ...
            for file_data in sorted(ai_data['files'], key=lambda x: x['issues_count'], reverse=True)[:10]:
                f.write(f"- `{file_data['file_name']}`: {file_data['issues_count']} issues\n")
    ...
    def _generate_machine_friendly_report(self, dead_letter_issues: List[Dict]) -> str:
        ...
        categorized_issues = {
            "summary": {
                "total_unfixable": len(dead_letter_issues),
                "categories": {}
            },
            "actionable_fixes": [],
            "complex_issues": [],
        ...
        for issue in dead_letter_issues:
            ...
            elif 'unknown' in message.lower() and ('role' in message.lower() or 'directive' in message.lower()):
        ...
        categorized_issues["summary"]["categories"] = {k: len(v) for k, v in by_source.items()}
    ...
    def _generate_human_friendly_report(self, dead_letter_issues: List[Dict]) -> str:
        ...
        for source, issues in sorted(by_source.items()):
            ...
            for issue in issues:
                message_key = issue.get('message', '')[:50] + "..." if len(issue.get('message', '')) > 50 else issue.get('message', '')
                by_message[message_key].append(issue)
            
            for message, message_issues in sorted(by_message.items(), key=lambda x: len(x[1]), reverse=True)[:10]:
        ...
        lines.append("-" * 50)
        lines.append("1. 🐍 Python Syntax Errors: Review code blocks for missing commas, quotes, brackets")
        lines.append("2. 📝 Unknown RST Constructs: Check for typos in :doc:, :ref:, or directive names")
        lines.append("3. 📏 Line Length: Break long lines at logical points (80-88 chars)")
        lines.append("4. 🔄 Duplicates: Rename or remove duplicate labels, targets, references")
        lines.append("5. 🔧 Markup Issues: Add blank lines before/after code blocks and sections")
        lines.append("")
        lines.append("🤖 For AI Assistant: Use 'generate_unfixable_issues_report(\"json\")' for structured data")
    ...
    def _get_suggested_approach(self, pattern: str) -> str:
        ...
        return approaches.get(pattern, f"Manual review and fixing required for {pattern}")
...
def generate_report(result: ValidationResult, format_type: str = "text") -> str:
    ...
    if format_type == "json":
        ...
        if result.issues:
            lines.append("\n❌ ISSUES FOUND:")
            for issue in result.issues:
                lines.append(f"  • {issue.file_path}:{issue.line_number} [{issue.level.value}] {issue.message}")
                if issue.suggestion:
...
Examples:
  ...
  %(prog)s validate --only eventtype      # Run specific validator
    ...
    check_parser.add_argument('--path', type=Path, default=Path('docs'), help='Path to check')
    check_parser.add_argument('--only', nargs='+', choices=['eventtype', 'rst_quality', 'code_examples', 'navigation', 'professional_rst', 'sphinx'], help='Run specific validators only')
    check_parser.add_argument('--max-workers', type=int, default=4, help='Maximum number of worker threads (default: 4)')
    
    # Fix command
    fix_parser = subparsers.add_parser('fix', help='Auto-fix documentation issues')
    fix_parser.add_argument('--path', type=Path, default=Path('docs'), help='Path to fix')
    fix_parser.add_argument('--only', nargs='+', choices=['eventtype', 'rst_quality', 'code_examples', 'navigation', 'professional_rst', 'sphinx'], help='Run specific validators only')
    fix_parser.add_argument('--max-workers', type=int, default=4, help='Maximum number of worker threads (default: 4)')
    fix_parser.add_argument('--no-export', action='store_true', help='Disable AI-consumable data export (default: export enabled)')
    
    # Report command
    report_parser = subparsers.add_parser('report', help='Generate quality report')
    report_parser.add_argument('--path', type=Path, default=Path('docs'), help='Path to analyze')
    report_parser.add_argument('--format', choices=['text', 'json'], default='text', help='Report format')
    report_parser.add_argument('--json', action='store_true', help='Output in JSON format (alias for --format json)')
    report_parser.add_argument('--quiet', action='store_true', help='Suppress progress output (errors only)')
    report_parser.add_argument('--verbose', action='store_true', help='Enable verbose output (debug level)')
    report_parser.add_argument('--only', nargs='+', choices=['eventtype', 'rst_quality', 'code_examples', 'navigation', 'professional_rst', 'sphinx'], help='Run specific validators only')
    report_parser.add_argument('--max-workers', type=int, default=4, help='Maximum number of worker threads (default: 4)')
    
    # Validate command (alias for check)
    validate_parser = subparsers.add_parser('validate', help='Validate documentation (alias for check)')
    validate_parser.add_argument('--path', type=Path, default=Path('docs'), help='Path to validate')
    validate_parser.add_argument('--only', nargs='+', choices=['eventtype', 'rst_quality', 'code_examples', 'navigation', 'professional_rst', 'sphinx'], help='Run specific validators only')
    validate_parser.add_argument('--max-workers', type=int, default=4, help='Maximum number of worker threads (default: 4)')
    
    # Unfixable issues report command
    unfixable_parser = subparsers.add_parser('unfixable', help='Generate report of unfixable issues from last run')
    unfixable_parser.add_argument('--format', choices=['human', 'json'], default='human', help='Report format (default: human)')
    unfixable_parser.add_argument('--output', type=Path, help='Output file path (default: stdout)')
    
    # Summary command for quick overview
    summary_parser = subparsers.add_parser('summary', help='Generate processing summary from last run')
    summary_parser.add_argument('--path', type=Path, default=Path('docs'), help='Path to analyze')
    summary_parser.add_argument('--only', nargs='+', choices=['eventtype', 'rst_quality', 'code_examples', 'navigation', 'professional_rst', 'sphinx'], help='Run specific validators only')
    summary_parser.add_argument('--max-workers', type=int, default=4, help='Maximum number of worker threads (default: 4)')
    
    # Handle case where no command is specified
    if len(sys.argv) == 1:
        # No arguments at all - default to check docs/
        sys.argv.extend(['check', '--path', 'docs'])
    elif len(sys.argv) == 2 and not sys.argv[1].startswith('-') and Path(sys.argv[1]).exists():
    ...
    if args.command == 'unfixable':
        ...
        if len(controller.shared_dead_letter_queue) > 0:
            ...
            logger.warning("⚠️  No unfixable issues data available. Run 'fix' command first.")
        sys.exit(0)
    ...
    if controller.fix_mode and hasattr(controller, 'shared_dead_letter_queue'):
        ...
        if unfixable_count > 0:
            ...
            logger.info(f"   Human-readable: python {Path(sys.argv[0]).name} unfixable --format human")
            logger.info(f"   AI-friendly JSON: python {Path(sys.argv[0]).name} unfixable --format json")
            logger.info(f"   Save to file: python {Path(sys.argv[0]).name} unfixable --format json --output unfixable-issues.json")
