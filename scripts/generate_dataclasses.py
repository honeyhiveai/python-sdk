...
def generate_dataclass(schema: Dict[str, Any], name: str, required_fields: List[str]) -> str:
    ...
    for field_name, field_schema in properties.items():
        ...
        if is_required:
            ...
            if field_type == 'List[Any]':
                lines.append(f'    {field_name}: {field_type} = field(default_factory=list)')
            elif field_type == 'Dict[str, Any]':
                lines.append(f'    {field_name}: {field_type} = field(default_factory=dict)')
            else:
    ...
    lines.append('                if hasattr(field_info.type, "__origin__") and field_info.type.__origin__ is type:')
    ...
    lines.append('                                    enum_class = base.__annotations__[key]')
    lines.append('                                    if hasattr(enum_class, "__origin__") and enum_class.__origin__ is Union:')
    lines.append('                                        enum_class = next((t for t in enum_class.__args__ if t != type(None)), None)')
    lines.append('                                    if enum_class and hasattr(enum_class, value):')
    lines.append('                                        data[key] = enum_class(value)')
...
def generate_models(openapi_path: str, output_path: str, include_validation: bool = False):
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Generate dataclass models from OpenAPI specification')
    parser.add_argument('input', help='Path to OpenAPI specification file (YAML or JSON)')
    parser.add_argument('output', help='Output path for generated Python file')
    parser.add_argument('--validation', action='store_true', help='Include validation utilities')
