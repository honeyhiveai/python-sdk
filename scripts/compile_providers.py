...
class ProviderCompiler:
    ...
    def _load_shared_configuration(self):
    ...
    def _load_all_providers(self, specific_provider: Optional[str] = None):
            ...
            for pattern_name, pattern_data in patterns.items():
                ...
                if signature_fields in inverted_index:
                    ...
                    if confidence > existing_conf:
                        inverted_index[signature_fields] = (provider_name, confidence)
                        logger.info(
    ...
    def _generate_extraction_function(
        ...
        for section_name, section_mappings in field_mappings.items():
            ...
            for field_name, field_config in section_mappings.items():
                ...
                function_lines.append(
                    f"        logger.debug(f'Failed to extract {field_name}: {{e}}')"
                )
    ...
    def _generate_field_extraction_code(
        ...
        if source_rule in transforms:
            transform = transforms[source_rule]
            ...
            return (
                f"_apply_transform('{implementation}', attributes, {repr(parameters)})"
            )
    ...
    def _compile_validation_rules(self) -> Dict[str, Any]:
        ...
        return self.shared_config.get("validation_rules", {})
    ...
    def _validate_bundle(self, bundle: CompiledProviderBundle):
    ...
    def _save_bundle(self, bundle: CompiledProviderBundle):
    ...
    def _log_compilation_stats(self):
...
def main():
    ...
    parser.add_argument(
        "--source-dir",
        type=Path,
        default=Path(__file__).parent.parent / "config" / "dsl",
        help="Source directory containing provider configurations",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
